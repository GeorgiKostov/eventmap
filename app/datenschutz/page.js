...
const COPY = {
  de: {
    ...
    processing: 'Was wir verarbeiten', processingItems: [
      ...
      <><b>Newsletter.</b> Nur wenn du dich anmeldest, speichern wir deine E-Mail-Adresse sowie – zur regionalen Auswahl der Events – die gewählte Ortsbezeichnung und deren ungefähre Mittelpunkt-Koordinaten (nicht dein Gerätestandort), deine gewählten Kategorien und deine Sprache. Wir nutzen Double-Opt-in: die Anmeldung ist erst nach Klick auf den Bestätigungslink aktiv. Als Nachweis deiner Einwilligung speichern wir den Zeitpunkt der Anmeldung, die Version des Einwilligungstextes und einen gehashten IP-Wert (nie die IP selbst). Abmeldung jederzeit über den Link in jeder E-Mail.</>,
  ...
  en: {
    ...
    processing: 'What we process', processingItems: [
      ...
      <><b>Newsletter.</b> If you subscribe, we store your email address and — to select events for your region — the chosen locality label and its approximate centre coordinates (not your device location), your chosen categories, and your language. We use double opt-in: the subscription is only active after you click the confirmation link. As proof of your consent we store the time of sign-up, the version of the consent text, and a hashed IP value (never the IP itself). Unsubscribe anytime via the link in every email.</>,
  ...
  bg: {
    ...
    processing: 'Какво обработваме', processingItems: [
      ...
      <><b>Бюлетин.</b> Ако се абонираш, пазим имейл адреса ти, както и — за да подбираме събития за твоя район — избраното име на населено място и приблизителните му централни координати (не местоположението на устройството ти), избраните категории и езика ти. Използваме двойно потвърждение (double opt-in): абонаментът е активен само след клик върху линка за потвърждение. Като доказателство за съгласието ти пазим момента на абониране, версията на текста за съгласие и хеширана стойност на IP адреса (никога самия IP). Можеш да се отпишеш по всяко време чрез линка във всеки имейл.</>,
