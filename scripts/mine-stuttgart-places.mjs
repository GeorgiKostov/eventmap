// Mine evergreen family places within an exact 40 km radius of Stuttgart
// from OpenStreetMap through the public Overpass API (ODbL).
//
// This is a deterministic, one-off bootstrap miner: one successful Overpass
// request, no LLM, no geocoder and no database writes. Coordinates are kept
// only when supplied by the OSM element itself (node coordinates or an
// Overpass-computed way/relation centre). Unknown facts stay null.
//
// Usage: node scripts/mine-stuttgart-places.mjs
import fs from 'fs';
import path from 'path';
import { crawlScope, distanceKm } from '../lib/crawl-scopes.js';

const SCOPE = crawlScope('stuttgart-40km');
const OUT = path.join(process.cwd(), 'data', 'mined', 'places-stuttgart-2026-07-13.json');
const ENDPOINTS = [
...
async function main() {
  ...
  const output = {
    _meta: {
      ...
      generator: 'scripts/mine-stuttgart-places.mjs',
