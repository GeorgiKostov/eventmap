...
// The exact storage shapes accepted at the DB write boundary. A valid calendar
// day may stand alone (the source published no time), or carry one valid HH:MM
// wall-clock time. Reject trailing offsets/seconds and impossible dates rather
// than letting malformed extractor output become durable data.
export function validLocalEventTime(value) {
  const raw = String(value || '');
  if (!validDateOf(raw)) return null;
  if (/^\d{4}-\d{2}-\d{2}$/.test(raw)) return raw;
  return validTimeOf(raw) ? raw : null;
}
