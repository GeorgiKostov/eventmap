...
// Service callers (for example the scheduled crawl) can authenticate without
// putting ADMIN_TOKEN in a URL. The header form keeps the existing one-click
// email links working while avoiding token leakage through request logs and
// Referer headers.
export function bearerTokenValid(req) {
  const expected = process.env.ADMIN_TOKEN || '';
  const header = req.headers?.get?.('authorization') || '';
  const given = header.startsWith('Bearer ') ? header.slice(7) : '';
  return expected.length >= 16 && safeEqual(given, expected);
}
...
//
// `tokenOnly` skips the cookie entirely: use it for state-mutating GET links
// (the one-click /api/admin/remove). A SameSite=Lax cookie IS sent on cross-site
// top-level GET navigation, so accepting the cookie on a mutating GET is a CSRF
// hole — a crafted link a logged-in admin clicks would fire with no secret. The
// email link always carries the token, so the cookie buys nothing there anyway.
export function isAdmin(req, { allowToken = false, tokenOnly = false } = {}) {
  if (!tokenOnly) {
