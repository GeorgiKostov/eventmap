...
// A start is either "YYYY-MM-DD" (no time published — see lib/event-time.js) or
// "YYYY-MM-DDTHH:MM". An end must carry a time, since a bare end date says nothing.
...
export function submissionProblem(body, kind, viennaToday) {
  ...
  if (kind === 'event') {
    ...
    if (body.ends_at != null && !DATETIME_RE.test(body.ends_at)) return 'bad_date_format';
  }
