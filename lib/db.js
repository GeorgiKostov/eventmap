...
// host's. A single-zone country (AT/BG) has one tz for every row, so this is
    ...
    WHERE status='published'
      ...
      AND (
        COALESCE(
          ...
          tz,
          CASE country WHEN 'AT' THEN 'Europe/Vienna' WHEN 'BG' THEN 'Europe/Sofia' ELSE 'UTC' END
        )
      ) < now()
  `;
  return res.count;
...
export async function publishedMapEvents() {
  ...
  const rows = await sql`
    SELECT id,kind,title,starts_at,ends_at,all_day,lat,lng,geo_precision,
           venue,address,town,categories,is_free,age_min,age_max,indoor,
           opening_hours,seasonal,src_kind
    FROM events WHERE status='published' ORDER BY starts_at ASC
  `;
  return rows.map(shape);
}
