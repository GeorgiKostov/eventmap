import postgres from 'postgres';
import { tzForEvent } from './geocode.js';
import { KID_PLACE_CATS } from './kid-cats.js';
import { cleanText } from './entities.js';
import { hasTime, validLocalEventTime } from './event-time.js';
import { isEventId } from './event-aliases.js';
import { rankPick, communityQualityGate, COMMUNITY_KINDS } from './source-quality.js';
// Re-exported for existing consumers; the definition lives with the trust tiers.
export { COMMUNITY_KINDS };

// Single-file data layer. Talks to Supabase Postgres over the transaction
// pooler (serverless-safe). The prototype's SQLite schema was designed to mirror
// this layout, so the port stayed mechanical.
//
// Isolation: our tables live in a dedicated `umkreis` schema inside a shared
// Supabase project; `search_path` pins unqualified queries to it. To graduate
// into a standalone project later, dump/restore that one schema — nothing else
// references it.
//
// Time rule (hard): starts_at/ends_at stay TEXT Vienna wall-clock strings
// ("2026-07-12T14:00"), never timestamptz — storing them as tz-aware types is
// exactly the drift bug in tasks/lessons.md. All "now/expiry" math uses
// viennaNow() and naive ::timestamp casts (no zone conversion).

const sql = postgres(process.env.DATABASE_URL || '', {
  ssl: 'require',
  prepare: false, // Supavisor transaction pooling doesn't support prepared statements
  // 'extensions' resolves PostGIS (ST_*, the geom column's && operator) without
  // schema-qualifying every call site — see scripts/migrate-viewport.mjs.
  connection: { search_path: 'umkreis, extensions' },
  idle_timeout: 20,
  max: 5,
});

// Postgres returns real booleans / text[]. Normalize reads back to the prototype's
// SQLite shape (1/0/null ints, categories array) so every consumer — the client
// app, JSON-LD, MCP server — keeps working unchanged.
function shape(r) {
  return {
    ...r,
    all_day: r.all_day ? 1 : 0,
    is_free: r.is_free == null ? null : r.is_free ? 1 : 0,
    indoor: r.indoor == null ? null : r.indoor ? 1 : 0,
    categories: r.categories || [],
    interest_count: r.interest_count ?? 0,
    report_flag: r.report_flag ?? null,
  };
}

// A data-quality report only becomes visible once this many INDEPENDENT reporters
// (distinct IP hashes) agree. One annoyed person cannot smear an event; three
// strangers reporting the same wrong time almost certainly means it is wrong.
export const REPORT_MIN = 3;
export const REACTION_KINDS = ['interest', 'cancelled', 'wrong_time', 'wrong_info', 'not_free'];

// Interest count + the single most-reported problem (only once it clears
// REPORT_MIN). Joined as grouped subqueries rather than per-row laterals so cost
// scales with the size of `reactions`, not with the thousands of published events.
const REACTION_JOIN = sql`
  LEFT JOIN (
    SELECT event_id, count(*)::int AS n FROM reactions WHERE kind='interest' GROUP BY event_id
  ) ri ON ri.event_id = e.id
  LEFT JOIN (
    SELECT DISTINCT ON (event_id) event_id, kind, count(*)::int AS n
    FROM reactions WHERE kind <> 'interest'
    GROUP BY event_id, kind
    ORDER BY event_id, count(*) DESC
  ) rr ON rr.event_id = e.id AND rr.n >= ${REPORT_MIN}
`;

// Public event rows deliberately exclude internal/write-only columns, most
// importantly the 768-dimension `embedding` vector. A wildcard event select
// made every sitemap/API/MCP read pull ~10 KB of unused vector text for each
// embedded event. Keep the projection centralized so a future internal column
// cannot silently become public egress again.
const PUBLIC_EVENT_COLUMNS = sql`
  e.id,e.kind,e.title,e.description,e.starts_at,e.ends_at,e.all_day,
  e.lat,e.lng,e.geo_precision,e.tz,e.venue,e.address,e.town,e.country,
  e.categories,e.is_free,e.age_min,e.age_max,e.indoor,e.emoji,e.photo_path,
  e.opening_hours,e.seasonal,e.status,e.src_kind,e.source_name,e.source_url,
  e.created_at,e.updated_at
`;

// Toggle an 'interest', or file a report. Idempotent per (event, kind, ip_hash) —
// the unique index is what makes one person one vote, so a retry or a double-tap
// can never inflate a counter. Returns the fresh interest count for the event.
export async function react(eventId, kind, ipHash, { on = true } = {}) {
  if (!REACTION_KINDS.includes(kind)) throw new Error(`bad reaction kind: ${kind}`);
  if (kind === 'interest' && !on) {
    await sql`DELETE FROM reactions WHERE event_id=${eventId} AND kind='interest' AND ip_hash=${ipHash}`;
  } else {
    await sql`
      INSERT INTO reactions (event_id, kind, ip_hash) VALUES (${eventId},${kind},${ipHash})
      ON CONFLICT (event_id, kind, ip_hash) DO NOTHING
    `;
  }
  const [row] = await sql`
    SELECT count(*)::int AS n FROM reactions WHERE event_id=${eventId} AND kind='interest'
  `;
  return row?.n ?? 0;
}

// Country -> IANA timezone. Every wall-clock "now" computation must go
// through one of these, never the host machine's local time or bare UTC
// (tasks/lessons.md — this class of bug bit us once already).
export const COUNTRY_TZ = { AT: 'Europe/Vienna', BG: 'Europe/Sofia', DE: 'Europe/Berlin' };

// "Now" as a wall-clock ISO string ('YYYY-MM-DDTHH:MM') in the given IANA
// timezone, so comparisons are correct regardless of the host machine's
// timezone.
export function nowInTz(tz) {
  const parts = new Intl.DateTimeFormat('en-CA', {
    timeZone: tz, hourCycle: 'h23',
    year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit',
  }).formatToParts(new Date());
  const g = (t) => parts.find((p) => p.type === t).value;
  return `${g('year')}-${g('month')}-${g('day')}T${g('hour')}:${g('minute')}`;
}
export function viennaNow() { return nowInTz(COUNTRY_TZ.AT); }
export function sofiaNow() { return nowInTz(COUNTRY_TZ.BG); }

// An event is "over" when ends_at passes; without an end time: 6h after start,
// or end of day for all-day events (design doc rule). Places (kind='place')
// are evergreen — no starts_at/ends_at — and never expire. Every row is
// compared against "now" in ITS OWN timezone (the per-event `tz` column,
// lib/geocode.js tzForEvent) — never a single country-wide zone, never the
// host's. A single-zone country (AT/BG/DE) has one tz for every row, so this is
// unchanged for them; a multi-zone country (US/RU/CA/AU/BR...) now expires a
// New York event and a Los Angeles event independently, DST-correct, because
// `AT TIME ZONE` converts each row's naive local wall-clock to an absolute
// instant in THAT row's zone before comparing to now(). tz falls back to
// COUNTRY_TZ by country, then UTC, for any row written before the tz column
// existed (belt-and-braces — migrate-event-tz.mjs already backfilled AT/BG).
export async function expireFinished(db = sql) {
  const res = await db`
    UPDATE events SET status='expired', updated_at=now()
    WHERE status='published'
      AND kind='event'
      AND (
        COALESCE(
          -- A date-only ends_at ("2026-12-31", length 10) is a known end DATE with
          -- no time (lib/event-time.js makeEndsAt) — it must live to the END of
          -- that day, not expire at its midnight. A timed ends_at is exact.
          -- pg_input_is_valid keeps one malformed source row from aborting the
          -- entire expiry sweep (and therefore every read behind the stale gate).
          CASE WHEN pg_input_is_valid(ends_at, 'timestamp') THEN
            CASE WHEN length(ends_at) = 10
                 THEN date_trunc('day', ends_at::timestamp) + interval '1 day'
                 ELSE ends_at::timestamp END
          END,
          -- length(starts_at)=10 means the source published no time (lib/event-time.js).
          -- Such a row must live until the END of its day: starts_at::timestamp is
          -- midnight, so the timed branch (+6h) would expire it at 06:00 on the
          -- very morning it happens.
          CASE WHEN pg_input_is_valid(starts_at, 'timestamp') THEN
            CASE WHEN all_day OR length(starts_at) = 10
                 THEN date_trunc('day', starts_at::timestamp) + interval '1 day'
                 ELSE starts_at::timestamp + interval '6 hours' END
          END
        ) AT TIME ZONE COALESCE(
          tz,
          CASE country
            WHEN 'AT' THEN 'Europe/Vienna'
            WHEN 'BG' THEN 'Europe/Sofia'
            WHEN 'DE' THEN 'Europe/Berlin'
            ELSE 'UTC'
          END
        )
      ) < now()
  `;
  return res.count;
}

let expireCheckPromise = null;
let nextLocalExpireCheckAt = 0;

// A read must not also pay for a full-table UPDATE. expireFinished() scans
// every published event; run it at most once per maxAgeSec, gated by a
// timestamp in `meta` (scripts/migrate-viewport.mjs), instead of on every
// page load. The freshness check itself is a plain absolute-duration
// comparison (elapsed wall-clock seconds since the last sweep) — not the
// per-event "is this event over" question, so it is not the tz-wall-clock
// rule (CLAUDE.md #3) that governs starts_at/ends_at comparisons below.
export async function expireIfStale(maxAgeSec = 600) {
  if (Date.now() < nextLocalExpireCheckAt) return;
  if (expireCheckPromise) return expireCheckPromise;

  expireCheckPromise = (async () => {
    const [row] = await sql`SELECT value FROM meta WHERE key='last_expire'`;
    // Stored as epoch ms — `now()::text` produced "2026-07-14 14:01:59+00",
    // which only parses via V8's lenient Date.parse (review finding #8).
    // Number() first; Date.parse fallback reads any old-format row once.
    const last = row ? (Number(row.value) || Date.parse(row.value) || 0) : 0;
    if (Date.now() - last <= maxAgeSec * 1000) {
      nextLocalExpireCheckAt = Date.now() + Math.min(60000, Math.max(5000, maxAgeSec * 500));
      return;
    }
    // Concurrent reads all cross the staleness threshold at once; without a lock
    // each would fire the full-table UPDATE (idempotent, but a thundering herd).
    // The update MUST run through `tx`: calling the pool-level helper from inside
    // this callback could land on a different Supavisor backend and escape the
    // transaction-scoped advisory lock.
    const swept = await sql.begin(async (tx) => {
      const [{ got }] = await tx`SELECT pg_try_advisory_xact_lock(hashtext('umkreis:last_expire')) AS got`;
      if (!got) return false;
      await expireFinished(tx);
      await tx`
        INSERT INTO meta (key, value)
        VALUES ('last_expire', ((extract(epoch FROM now()) * 1000)::bigint)::text)
        ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
      `;
      return true;
    });
    nextLocalExpireCheckAt = Date.now() + (swept ? 60000 : 5000);
  })();

  try {
    await expireCheckPromise;
  } finally {
    expireCheckPromise = null;
  }
}

// Claim one globally serialized slot for a rate-limited external service. The
// module-level promise in lib/geocode.js protects one warm function instance;
// this database timestamp protects ALL serverless instances. The transaction
// never sleeps while holding its advisory lock: callers wait outside, retry,
// and exactly one of them advances the timestamp.
export async function claimExternalRateSlot(key, minIntervalMs) {
  const interval = Math.max(0, Math.ceil(Number(minIntervalMs) || 0));
  return sql.begin(async (tx) => {
    await tx`SELECT pg_advisory_xact_lock(hashtextextended(${`external-rate|${key}`}, 0))`;
    const [row] = await tx`
      SELECT
        (SELECT value FROM meta WHERE key=${`external-rate:${key}`}) AS value,
        ((extract(epoch FROM clock_timestamp()) * 1000)::bigint)::text AS now_ms
    `;
    const nowMs = Number(row.now_ms);
    const waitMs = Math.max(0, Number(row.value || 0) + interval - nowMs);
    if (waitMs > 0) return waitMs;
    await tx`
      INSERT INTO meta (key, value) VALUES (${`external-rate:${key}`}, ${String(nowMs)})
      ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value
    `;
    return 0;
  });
}

export async function publishedEvents() {
  await expireIfStale();
  const rows = await sql`
    SELECT ${PUBLIC_EVENT_COLUMNS}, ri.n AS interest_count, rr.kind AS report_flag
    FROM events e ${REACTION_JOIN}
    WHERE e.status='published' ORDER BY e.starts_at ASC
  `;
  return rows.map(shape);
}

// Public JSON API: cursor-bounded by id so one request can never dump the
// catalog. `limit + 1` is the standard keyset-pagination lookahead; the extra
// row is never returned to the caller.
export async function publishedEventsPage({ afterId, limit = 100 } = {}) {
  await expireIfStale();
  const pageLimit = Math.max(1, Math.min(100, Number(limit) || 100));
  const rows = await sql`
    SELECT ${PUBLIC_EVENT_COLUMNS}, ri.n AS interest_count, rr.kind AS report_flag
    FROM events e ${REACTION_JOIN}
    WHERE e.status='published'
      ${afterId ? sql`AND e.id > ${afterId}` : sql``}
    ORDER BY e.id
    LIMIT ${pageLimit + 1}
  `;
  const hasMore = rows.length > pageLimit;
  const page = rows.slice(0, pageLimit).map(shape);
  return {
    events: page,
    nextCursor: hasMore && page.length ? String(page[page.length - 1].id) : null,
  };
}

// The sitemap is a priority feed, not a catalog dump. Submit only described,
// source-backed events in the explicit SEO city catchments; all other public
// events remain reachable through the product without asking Google to spend
// crawl demand on thin or out-of-rollout leaf pages. The result projection is
// still only id + timestamp, never event bodies or embeddings.
export async function sitemapEvents({ cities = [] } = {}) {
  await expireIfStale();
  if (!cities.length) return [];
  if (cities.length > 20) throw new Error('Event sitemap is limited to 20 city catchments');
  return await sql`
    WITH cities AS (
      SELECT * FROM (VALUES ${sql(cities.map((city) => [
        city.lat,
        city.lng,
        city.radiusKm,
      ]))}) AS c(lat, lng, radius_km)
    ), reports AS (
      SELECT event_id
      FROM reactions
      WHERE kind <> 'interest'
      GROUP BY event_id, kind
      HAVING count(*) >= ${REPORT_MIN}
    )
    SELECT DISTINCT e.id, e.updated_at
    FROM events e
    JOIN cities c ON
      e.geom && ST_Expand(
        ST_SetSRID(ST_MakePoint(c.lng::float8, c.lat::float8), 4326),
        c.radius_km::float8 / 111
      )
      AND ST_DWithin(
        e.geom::geography,
        ST_SetSRID(ST_MakePoint(c.lng::float8, c.lat::float8), 4326)::geography,
        c.radius_km::float8 * 1000
      )
    LEFT JOIN reports r ON r.event_id = e.id
    WHERE e.status='published'
      AND e.kind='event'
      AND e.starts_at ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}'
      AND length(trim(COALESCE(e.description, ''))) >= 30
      AND e.source_url IS NOT NULL
      AND r.event_id IS NULL
    ORDER BY e.id
  `;
}

// ---- viewport-native map (briefs/viewport-rebuild-brief.md) ----
// Filter semantics are copied 1:1 from app/page.js's commonFiltered /
// filteredEvents memos — same boundary values, not reimplemented from
// scratch. See that file's `dFrom`/`dTo` memo and the predicates chained
// below it for the source of truth.
//
// `when` window: the client computes dFrom/dTo as Vienna-pinned, DATE-ONLY
// strings ('YYYY-MM-DD', via todayStr()/addDays()'s en-CA Intl formatting) —
// not 'YYYY-MM-DDTHH:MM' wall-clock timestamps. The overlap test compares
// only the first 10 chars of starts_at/ends_at (ev.starts_at.slice(0,10)),
// so we mirror that with left(...,10) rather than comparing full text against
// a date-only bound (which would wrongly exclude same-day events with a time
// component, e.g. '2026-07-20T09:00' > '2026-07-20'). Applies to kind='event'
// only — places are evergreen and always pass (design doc rule; also copied
// verbatim in commonFiltered's date filter, which only appears inside
// filteredEvents).
function commonFilters({ kind, cats, free, kids, community, inout, tod, when, source } = {}) {
  return sql`
    ${kind ? sql`AND e.kind = ${kind}` : sql``}
    ${source ? sql`AND e.source_name = ${source}` : sql``}
    ${cats && cats.length ? sql`AND e.categories && ${cats}` : sql``}
    ${free ? sql`AND e.is_free = true` : sql``}
    ${kids ? sql`AND (e.age_min IS NOT NULL OR e.age_max IS NOT NULL
                      OR 'family' = ANY(e.categories)
                      OR e.categories && ${KID_PLACE_CATS})` : sql``}
    ${community ? sql`AND e.src_kind IN ${sql([...COMMUNITY_KINDS])}` : sql``}
    ${inout === 'in' ? sql`AND e.indoor = true` : sql``}
    ${inout === 'out' ? sql`AND e.indoor = false` : sql``}
    ${when?.from && when?.to ? sql`
      AND (e.kind <> 'event' OR (
        left(e.starts_at, 10) <= ${when.to}
        AND left(COALESCE(e.ends_at, e.starts_at), 10) >= ${when.from}
      ))
    ` : sql``}
    ${tod && tod.length ? sql`
      AND (e.kind <> 'event' OR e.all_day OR length(e.starts_at) = 10 OR (
        CASE WHEN COALESCE(NULLIF(substr(e.starts_at, 12, 2), '')::int, 0) < 12 THEN 'morning'
             WHEN COALESCE(NULLIF(substr(e.starts_at, 12, 2), '')::int, 0) < 17 THEN 'afternoon'
             ELSE 'evening' END
      ) = ANY(${tod})
      )
    ` : sql``}
  `;
}

// Active-highlight lookup (paid/editorial placement, db/schema.sql
// `highlights`): picks the single strongest highlight period OVERLAPPING
// [from, to] — gold outranks editorial — so a row carries at most one tier.
// Dates are Vienna wall-clock date-only strings (viennaNow().slice(0,10) for
// the live surfaces), never host/UTC, per CLAUDE.md rule 3.
//
// `to` defaults to `from`, which makes the common case a point-in-time "active
// today" test — that is what every live surface (map, search, detail) wants.
// The digest is the one caller that needs a real window: it is built on
// Thursday and FROZEN, so asking "active today" there would bake Thursday's
// answer into a snapshot describing Fri–Sun and silently drop a gold period
// that only covers the weekend itself.
function highlightJoin(from, to = from) {
  return sql`
    LEFT JOIN LATERAL (
      SELECT h.tier FROM highlights h
      WHERE h.event_id = e.id AND h.starts_at <= ${to} AND h.ends_at >= ${from}
      ORDER BY CASE h.tier WHEN 'gold' THEN 0 WHEN 'editorial' THEN 1 ELSE 2 END
      LIMIT 1
    ) hl ON true
  `;
}

// bbox = [minLng, minLat, maxLng, maxLat]. LIMIT 800 with `total` (unlimited
// count) + `truncated` so the client can show a "zoom in / narrow filters"
// hint at Vienna-city density. Places sort FIRST: they're evergreen and few
// (design rule: never hidden), so the flood of timed events must not evict
// them past the cap — central Vienna is 1,029 events + 37 places, and
// starts_at ordering alone starved every place out of the response (review
// finding #1). Within each kind, soonest events first, as before. Reaction
// aggregates are evaluated only for the bounded result set below —
// never by grouping the entire reactions table on every map movement.
export async function mapPins({ bbox, ...filters } = {}) {
  await expireIfStale();
  const today = viennaNow().slice(0, 10);
  const [minLng, minLat, maxLng, maxLat] = bbox;
  const where = sql`
    e.status='published'
    AND e.geom && ST_MakeEnvelope(${minLng}, ${minLat}, ${maxLng}, ${maxLat}, 4326)
    ${commonFilters(filters)}
  `;
  // Places sort first so the timed-event flood can't evict them, but that same
  // rule means >800 places in a viewport would evict every EVENT — the reverse
  // starvation. Reserve the cap: places take at most PLACE_CAP slots, so events
  // are guaranteed the remainder (800 - min(places, PLACE_CAP)). Today's per-city
  // place counts are in the tens; this only bites as the OSM place catalog grows.
  const PLACE_CAP = 300;
  const rows = await sql`
    WITH ranked AS (
      SELECT e.id,e.kind,e.title,e.starts_at,e.ends_at,e.all_day,e.lat,e.lng,e.geo_precision,
             e.venue,e.address,e.town,e.country,e.categories,e.is_free,e.age_min,e.age_max,e.indoor,
             e.opening_hours,e.seasonal,e.src_kind,
             hl.tier AS highlight,
             (count(*) OVER ())::int AS total,
             -- Active highlight sorts first WITHIN its partition too: a paid/
             -- editorial pin must never be the one the PLACE_CAP trims off.
             row_number() OVER (
               PARTITION BY (e.kind = 'place')
               ORDER BY (hl.tier IS NOT NULL) DESC, e.starts_at ASC NULLS LAST
             ) AS rn
      FROM events e ${highlightJoin(today)}
      WHERE ${where}
    ), limited AS (
      SELECT id,kind,title,starts_at,ends_at,all_day,lat,lng,geo_precision,
             venue,address,town,country,categories,is_free,age_min,age_max,indoor,
             opening_hours,seasonal,src_kind,highlight,total
      FROM ranked
      WHERE kind <> 'place' OR rn <= ${PLACE_CAP}
      -- Highlight is the FIRST sort key, ahead of place-then-event: an active
      -- highlight must never be silently dropped by the LIMIT 800 cap either
      -- (paid pins don't get to vanish just because the viewport is dense).
      ORDER BY (highlight IS NOT NULL) DESC, (kind = 'place') DESC, starts_at ASC NULLS LAST
      LIMIT 800
    )
    SELECT l.*,
           COALESCE(ri.n, 0)::int AS interest_count,
           rr.kind AS report_flag
    FROM limited l
    LEFT JOIN LATERAL (
      SELECT count(*)::int AS n
      FROM reactions r
      WHERE r.event_id=l.id AND r.kind='interest'
    ) ri ON true
    LEFT JOIN LATERAL (
      SELECT r.kind
      FROM reactions r
      WHERE r.event_id=l.id AND r.kind <> 'interest'
      GROUP BY r.kind
      HAVING count(*) >= ${REPORT_MIN}
      ORDER BY count(*) DESC
      LIMIT 1
    ) rr ON true
    ORDER BY (l.highlight IS NOT NULL) DESC, (l.kind = 'place') DESC, l.starts_at ASC NULLS LAST
  `;
  const total = rows[0]?.total ?? 0;
  const events = rows.map(({ total: _total, ...row }) => shape(row));
  return { events, total, truncated: total > events.length };
}

// Zoomed-out overview: server-side spatial binning so a whole-country view
// never ships per-event rows. No REACTION_JOIN — cells don't show
// interest/report state, only a count.
export async function mapCells({ bbox, cellDeg, ...filters } = {}) {
  await expireIfStale();
  const [minLng, minLat, maxLng, maxLat] = bbox;
  const where = sql`
    e.status='published'
    AND e.geom && ST_MakeEnvelope(${minLng}, ${minLat}, ${maxLng}, ${maxLat}, 4326)
    ${commonFilters(filters)}
  `;
  const rows = await sql`
    SELECT avg(e.lat)::float8 AS lat, avg(e.lng)::float8 AS lng, count(*)::int AS n,
           (sum(count(*)) OVER ())::int AS total
    FROM events e
    WHERE ${where}
    GROUP BY floor(e.lat / ${cellDeg}), floor(e.lng / ${cellDeg})
  `;
  const total = rows[0]?.total ?? 0;
  const cells = rows.map(({ total: _total, ...cell }) => cell);
  return { cells, total };
}

// Global text search (title/venue/town), independent of the viewport — the
// search box looks up events anywhere, not just what's on screen. `%`/`_`
// are escaped so a user typing them can't turn a substring match into a
// wildcard scan; ILIKE's default escape char is backslash.
//
// unaccent() folds diacritics on BOTH sides so "munchen" finds "München" and
// "cafe" finds "Café" — a raw ILIKE returns zero for the accented form, which
// is exactly what a user who can't type umlauts gets. (unaccent folds within a
// script only; Latin→Cyrillic transliteration is a separate unsolved problem.)
export async function searchEvents(q) {
  const today = viennaNow().slice(0, 10);
  const escaped = String(q).replace(/[%_\\]/g, (c) => `\\${c}`);
  const pattern = `%${escaped}%`;
  const prefix = `${escaped}%`;
  return await sql`
    SELECT e.id, e.kind, e.title, e.starts_at, e.venue, e.town, e.lat, e.lng, hl.tier AS highlight
    FROM events e ${highlightJoin(today)}
    WHERE e.status='published'
      AND search_normalize(
        COALESCE(e.title, '') || ' ' || COALESCE(e.venue, '') || ' ' || COALESCE(e.town, '')
      ) LIKE search_normalize(${pattern})
    -- Relevance before recency: a title that STARTS with the query, then any
    -- title match, then venue/town-only matches — otherwise a common substring
    -- ("park") buries an exact title hit past the 20-row cap on date alone.
    ORDER BY (search_normalize(e.title) LIKE search_normalize(${prefix})) DESC,
             (search_normalize(e.title) LIKE search_normalize(${pattern})) DESC,
             e.starts_at ASC NULLS LAST
    LIMIT 20
  `;
}

// MCP search is a bounded SQL query, not an in-memory filter over the entire
// catalog. The window count preserves the tool's `total` result without a
// second round trip; only `limit` public rows cross the network.
export async function searchEventCatalog({
  query, dateFrom, dateTo, category, town, freeOnly, forKids,
  nearLat, nearLng, maxKm, limit = 25,
} = {}) {
  await expireIfStale();
  const pageLimit = Math.max(1, Math.min(100, Number(limit) || 25));
  const text = String(query || '').trim();
  const escaped = text.replace(/[%_\\]/g, (c) => `\\${c}`);
  const pattern = `%${escaped}%`;
  const radius = Number(maxKm);
  const useRadius = Number.isFinite(radius) && radius > 0;
  const lat = Number.isFinite(Number(nearLat)) ? Number(nearLat) : 48.3;
  const lng = Number.isFinite(Number(nearLng)) ? Number(nearLng) : 14.29;
  const point = sql`ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326)`;

  const rows = await sql`
    SELECT ${PUBLIC_EVENT_COLUMNS}, count(*) OVER()::int AS total_count
    FROM events e
    WHERE e.status='published' AND e.kind='event'
      ${dateFrom ? sql`AND left(COALESCE(e.ends_at, e.starts_at), 10) >= ${dateFrom}` : sql``}
      ${dateTo ? sql`AND left(e.starts_at, 10) <= ${dateTo}` : sql``}
      ${category ? sql`AND ${category} = ANY(e.categories)` : sql``}
      ${town ? sql`AND lower(e.town) = lower(${town})` : sql``}
      ${freeOnly ? sql`AND e.is_free = true` : sql``}
      ${forKids ? sql`AND (e.age_min IS NOT NULL OR 'family' = ANY(e.categories))` : sql``}
      ${text ? sql`
        AND (
          unaccent(COALESCE(e.title, '')) ILIKE unaccent(${pattern})
          OR unaccent(COALESCE(e.description, '')) ILIKE unaccent(${pattern})
          OR unaccent(COALESCE(e.venue, '')) ILIKE unaccent(${pattern})
          OR unaccent(COALESCE(e.town, '')) ILIKE unaccent(${pattern})
        )
      ` : sql``}
      ${useRadius ? sql`
        AND e.geom && ST_Expand(${point}, ${radius / 111})
        AND ST_DWithin(e.geom::geography, ${point}::geography, ${radius * 1000})
      ` : sql``}
    ORDER BY e.starts_at ASC
    LIMIT ${pageLimit}
  `;
  const total = rows.length ? Number(rows[0].total_count) : 0;
  return {
    total,
    events: rows.map(({ total_count: _totalCount, ...row }) => shape(row)),
  };
}

// Server-rendered city and city-month landing pages. Date bounds are local
// wall-clock date strings, matching starts_at/ends_at storage. Spatial matching
// makes the page useful for a city catchment without relying on inconsistent
// source spellings in e.town. Reported rows stay off acquisition pages.
function shiftIsoDate(date, days) {
  const [year, month, day] = String(date).split('-').map(Number);
  return new Date(Date.UTC(year, month - 1, day + days)).toISOString().slice(0, 10);
}

export async function seoEvents({ label, lat, lng, radiusKm, from, to, kids = false, limit = 80 }) {
  await expireIfStale();
  const pageLimit = Math.max(1, Math.min(100, Number(limit) || 80));
  const point = sql`ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326)`;
  const overlapFrom = shiftIsoDate(from, -45);
  const rows = await sql`
    WITH matched AS (
      SELECT e.id,e.title,e.starts_at,e.ends_at,e.all_day,e.venue,e.town,e.categories,e.is_free,
             e.age_min,e.age_max,e.source_name,e.updated_at,
             left(e.starts_at, 10) < ${from} AS ongoing,
             COALESCE(
               lower(e.town) = lower(${label}) OR lower(e.town) LIKE lower(${label}) || '-%',
               false
             ) AS in_city,
             ST_Distance(e.geom::geography, ${point}::geography) / 1000 AS distance_km,
             row_number() OVER (
               PARTITION BY lower(e.title), COALESCE(NULLIF(lower(e.venue), ''), lower(e.town), '')
               ORDER BY e.starts_at
             ) AS series_rank
      FROM events e ${REACTION_JOIN}
      WHERE e.status='published' AND e.kind='event'
        AND rr.event_id IS NULL
        ${kids ? sql`AND (e.age_min IS NOT NULL OR e.age_max IS NOT NULL OR 'family' = ANY(e.categories))` : sql``}
        AND (
          left(e.starts_at, 10) BETWEEN ${from} AND ${to}
          OR (
            left(e.starts_at, 10) BETWEEN ${overlapFrom} AND ${shiftIsoDate(from, -1)}
            AND left(COALESCE(e.ends_at, e.starts_at), 10) >= ${from}
            AND left(e.starts_at, 10) ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
            AND left(e.ends_at, 10) ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
            AND left(e.ends_at, 10)::date - left(e.starts_at, 10)::date BETWEEN 0 AND 120
          )
        )
        AND e.geom && ST_Expand(${point}, ${radiusKm / 111})
        AND ST_DWithin(e.geom::geography, ${point}::geography, ${radiusKm * 1000})
    )
    SELECT id,title,starts_at,ends_at,all_day,venue,town,categories,is_free,source_name,ongoing,
           round(distance_km::numeric, 1)::float AS distance_km,
           count(*) OVER()::int AS total_count,
           count(*) FILTER (WHERE is_free) OVER()::int AS free_count,
           count(*) FILTER (
             WHERE age_min IS NOT NULL OR age_max IS NOT NULL OR 'family' = ANY(categories)
           ) OVER()::int AS kids_count,
           max(updated_at) OVER() AS last_modified
    FROM matched
    WHERE series_rank = 1
    ORDER BY in_city DESC, ongoing, starts_at, distance_km
    LIMIT ${pageLimit}
  `;
  const total = rows.length ? Number(rows[0].total_count) : 0;
  return {
    total,
    facets: {
      free: rows.length ? Number(rows[0].free_count) : 0,
      kids: rows.length ? Number(rows[0].kids_count) : 0,
    },
    lastModified: rows[0]?.last_modified || null,
    events: rows.map(({
      total_count: _totalCount,
      free_count: _freeCount,
      kids_count: _kidsCount,
      last_modified: _lastModified,
      ...row
    }) => shape(row)),
  };
}

// One bounded query for sitemap month inclusion. Callers provide the explicit
// SEO city registry and moving month windows; the cap prevents this helper from
// becoming an accidental arbitrary spatial reporting endpoint.
export async function seoWindowManifest({ cities, windows }) {
  const cityWindows = cities.flatMap((city) => windows.map((window) => ({ city, window })));
  if (!cityWindows.length) return [];
  if (cityWindows.length > 120) throw new Error('SEO manifest is limited to 120 windows');

  const rows = await sql`
    WITH windows AS (
      SELECT * FROM (VALUES ${sql(cityWindows.map(({ city, window }) => [
        city.slug,
        city.lat,
        city.lng,
        city.radiusKm,
        window.key,
        window.from,
        window.to,
        shiftIsoDate(window.from, -45),
        Boolean(window.kids),
      ]))}) AS w(city_slug, lat, lng, radius_km, window_key, date_from, date_to, overlap_from, kids)
    ), reports AS (
      SELECT event_id
      FROM reactions
      WHERE kind <> 'interest'
      GROUP BY event_id, kind
      HAVING count(*) >= ${REPORT_MIN}
    )
    SELECT w.city_slug, w.window_key,
           count(DISTINCT (
             lower(e.title), COALESCE(NULLIF(lower(e.venue), ''), lower(e.town), '')
           )) FILTER (WHERE r.event_id IS NULL)::int AS event_count,
           max(e.updated_at) FILTER (WHERE r.event_id IS NULL) AS last_modified
    FROM windows w
    LEFT JOIN events e ON e.status='published' AND e.kind='event'
      AND (
        left(e.starts_at, 10) BETWEEN w.date_from AND w.date_to
        OR (
          left(e.starts_at, 10) BETWEEN w.overlap_from AND to_char(w.date_from::date - 1, 'YYYY-MM-DD')
          AND left(COALESCE(e.ends_at, e.starts_at), 10) >= w.date_from
          AND left(e.starts_at, 10) ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
          AND left(e.ends_at, 10) ~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'
          AND left(e.ends_at, 10)::date - left(e.starts_at, 10)::date BETWEEN 0 AND 120
        )
      )
      AND (NOT w.kids::boolean OR e.age_min IS NOT NULL OR e.age_max IS NOT NULL OR 'family' = ANY(e.categories))
      AND e.geom && ST_Expand(ST_SetSRID(ST_MakePoint(w.lng::float8, w.lat::float8), 4326), w.radius_km::float8 / 111)
      AND ST_DWithin(
        e.geom::geography,
        ST_SetSRID(ST_MakePoint(w.lng::float8, w.lat::float8), 4326)::geography,
        w.radius_km::float8 * 1000
      )
    LEFT JOIN reports r ON r.event_id = e.id
    GROUP BY w.city_slug, w.window_key
    ORDER BY w.city_slug, w.window_key
  `;
  return rows;
}

export async function seoMonthManifest({ cities, months }) {
  return seoWindowManifest({
    cities,
    windows: months.map((month) => ({ key: month.slug, from: month.from, to: month.to })),
  });
}

// Resolve the saved-events list. Deliberately NOT scoped to the viewport —
// saved events are usually elsewhere on the map (brief: don't let "outside
// the current bbox" look like "gone").
export async function eventsByIds(ids) {
  if (!ids || !ids.length) return [];
  const today = viennaNow().slice(0, 10);
  const rows = await sql`
    SELECT e.id,e.kind,e.title,e.starts_at,e.ends_at,e.all_day,e.lat,e.lng,e.geo_precision,
           e.venue,e.address,e.town,e.country,e.categories,e.is_free,e.age_min,e.age_max,e.indoor,
           e.opening_hours,e.seasonal,e.src_kind,
           ri.n AS interest_count, rr.kind AS report_flag, hl.tier AS highlight
    FROM events e ${REACTION_JOIN} ${highlightJoin(today)}
    WHERE e.status='published' AND e.id IN ${sql(ids.slice(0, 100))}
    ORDER BY e.starts_at ASC NULLS LAST
  `;
  return rows.map(shape);
}

// ---- account-scoped contributions + favourites ----
// Auth remains provider-neutral here: callers pass a verified UUID from the
// Supabase JWT, and the data layer never reads cookies or provider metadata.
const CONTRIBUTION_KINDS = new Set(['manual', 'photo', 'link']);

export async function isActiveAuthSession(sessionId, userId) {
  const rows = await sql`
    SELECT 1 FROM auth.sessions
    WHERE id=${sessionId}::uuid AND user_id=${userId}::uuid
    LIMIT 1
  `;
  return rows.length > 0;
}

export async function recordEventContribution(userId, eventId, contributionKind, { merged = false } = {}) {
  if (!CONTRIBUTION_KINDS.has(contributionKind)) throw new Error('bad contribution kind');
  await sql`
    INSERT INTO event_contributions (user_id,event_id,contribution_kind,merged)
    VALUES (${userId},${eventId},${contributionKind},${!!merged})
    ON CONFLICT (user_id,event_id) DO UPDATE SET
      contribution_kind=EXCLUDED.contribution_kind,
      merged=(event_contributions.merged OR EXCLUDED.merged),
      updated_at=now()
  `;
}

export async function userCanEditContribution(userId, eventId) {
  const rows = await sql`
    SELECT 1 FROM event_contributions
    WHERE user_id=${userId} AND event_id=${eventId} AND merged=false
    LIMIT 1
  `;
  return rows.length > 0;
}

export async function userContributions(userId, limit = 50) {
  const capped = Math.max(1, Math.min(100, Number(limit) || 50));
  const rows = await sql`
    SELECT e.id,e.kind,e.title,e.starts_at,e.town,e.status,
           c.contribution_kind,c.merged,c.created_at,c.updated_at
    FROM event_contributions c
    JOIN events e ON e.id=c.event_id
    WHERE c.user_id=${userId}
    ORDER BY c.updated_at DESC
    LIMIT ${capped}
  `;
  return rows.map((row) => ({ ...row, merged: !!row.merged }));
}

export async function userFavoriteIds(userId) {
  const rows = await sql`
    SELECT f.event_id
    FROM user_favorites f
    JOIN events e ON e.id=f.event_id
    WHERE f.user_id=${userId} AND e.status='published'
    ORDER BY f.saved_at ASC
    LIMIT 100
  `;
  return rows.map((row) => String(row.event_id));
}

export async function mergeUserFavorites(userId, ids) {
  const clean = [...new Set((ids || []).map(String).filter((id) => /^\d+$/.test(id)))].slice(0, 100);
  if (clean.length) {
    await sql`
      INSERT INTO user_favorites (user_id,event_id)
      SELECT ${userId},e.id FROM events e
      WHERE e.status='published' AND e.id IN ${sql(clean)}
      ON CONFLICT (user_id,event_id) DO NOTHING
    `;
  }
  return userFavoriteIds(userId);
}

export async function setUserFavorite(userId, eventId, on) {
  if (on) {
    await sql`
      INSERT INTO user_favorites (user_id,event_id)
      SELECT ${userId},e.id FROM events e
      WHERE e.id=${eventId} AND e.status='published'
      ON CONFLICT (user_id,event_id) DO NOTHING
    `;
  } else {
    await sql`DELETE FROM user_favorites WHERE user_id=${userId} AND event_id=${eventId}`;
  }
  return userFavoriteIds(userId);
}

// Resolve which frozen digest picks still have a public landing page. Keep this
// separate from eventsByIds(): saved pins and the map are published-only, while
// a weekend archive may link to a naturally expired event archive. Deliberately
// return only the fields the caller needs and cap the batch like eventsByIds().
export async function eventLandingStatuses(ids) {
  if (!ids || !ids.length) return [];
  await expireIfStale();
  return await sql`
    SELECT e.id,e.status
    FROM events e
    WHERE e.id IN ${sql(ids.slice(0, 100))}
      AND (e.status='published' OR (e.status='expired' AND e.kind='event'))
  `;
}

// Weekend digest picks (docs/ops/weekly-automation.md): the best family events
// around a point for a Fri–Sun window. `from`/`to` are LOCAL date-only strings
// (starts_at is local wall-clock TEXT — string compare, never tz math).
// DISTINCT ON collapses a series (same title, several dates/venues in range) to
// its earliest occurrence so one Ferienprogramm can't fill the whole digest.
//
// A REPORTED event (>=REPORT_MIN independent reporters — REACTION_JOIN's rr)
// is excluded in the WHERE clause, before DISTINCT ON picks the "earliest"
// row: if a series' earliest occurrence is the reported one but a later date
// is fine, filtering pre-DISTINCT lets that later date surface instead of
// dropping the whole series. Never feature a reported event in the digest.
//
// Ranking happens in JS on the small candidate set via rankPick() (one shared
// definition, lib/source-quality.js — also used by the "why did this rank
// here" reasoning elsewhere, so there is only one tuple to keep in sync).
// LEXICOGRAPHIC, not an additive sum: an additive sum once let three lower
// signals (free+community+precise = 5) outrank the product lens itself
// (family = 4), so a non-family event could headline the family digest
// (tasks/lessons.md, 2026-07-14). As a tuple: family fit (strictly dominant)
// → source tier → precise location → free → interest_count → soonest.
//
// Source tier REPLACES "community-submitted" as a ranking boost: George asked
// to rate official sources (linztermine etc) higher, and letting community
// submissions rank ABOVE generic crawl was exactly backwards for a validation
// -stage product where trust matters more than rewarding a contributor — a
// single test row ("Test event") once headlined the first digest run because
// nothing gated or deprioritized unvetted content (same lesson entry). Now
// community content is the LOWEST tier and only survives at all if it passes
// communityQualityGate (venue + real description + no reports) — the "no
// random shit" gate, applied in JS below since it needs the report_flag this
// query already joins.
// Highlights are SELECTED here but deliberately DO NOT rank (George, 2026-07-16:
// "badge only, no rank change"). Payment buys visibility in the digest — the
// treatment + „Anzeige" label — never a slot and never a position: rankPick is
// untouched, so a gold event earns its place in the mail on family fit and
// source quality exactly like an organic one, or it doesn't appear at all.
//
// This is a deliberate DIVERGENCE from mapPins, where an active highlight IS the
// first sort key and is cap-exempt — not drift. The map is a spatial surface
// where a paid pin must not be trimmed off by a dense viewport; the digest is an
// editorial pick of the weekend's best family events, and letting payment reorder
// it would both contradict the family-first thesis and make the P2B Art. 5
// ranking-disclosure page due (docs/decisions/2026-07-12-paid-placement-compliance.md).
export async function weekendPicks({ lat, lng, radiusKm, from, to, limit = 10 }) {
  await expireIfStale();
  const rows = await sql`
    SELECT DISTINCT ON (lower(e.title))
           e.id, e.title, e.description, e.starts_at, e.ends_at, e.all_day,
           e.venue, e.address, e.town, e.categories, e.is_free, e.age_min, e.age_max,
           e.indoor, e.geo_precision, e.src_kind, e.source_name, e.source_url, e.lat, e.lng,
           e.updated_at,
           ri.n AS interest_count, rr.kind AS report_flag, hl.tier AS highlight
    FROM events e ${REACTION_JOIN} ${highlightJoin(from, to)}
    WHERE e.status='published' AND e.kind='event'
      -- Never feature a reported event — see comment above on why this must
      -- run before DISTINCT ON, not after.
      AND rr.event_id IS NULL
      -- Overlap, not start-only: a festival that began before Friday but runs
      -- through the weekend is happening, so it's pickable (mirrors mapPins).
      AND left(e.starts_at, 10) <= ${to}
      AND left(COALESCE(e.ends_at, e.starts_at), 10) >= ${from}
      AND ST_DWithin(e.geom::geography,
                     ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326)::geography,
                     ${radiusKm * 1000})
    ORDER BY lower(e.title), e.starts_at ASC
  `;
  return rows
    .map(shape)
    .filter(communityQualityGate)
    .sort((a, b) => {
      const ra = rankPick(a), rb = rankPick(b);
      for (let i = 0; i < ra.length; i++) if (rb[i] !== ra[i]) return rb[i] - ra[i];
      return String(a.starts_at).localeCompare(String(b.starts_at));
    })
    .slice(0, limit);
}

// Exact reviewed rows for a frozen editorial weekend. Unlike weekendPicks(),
// this does not collapse equal-title series or rank candidates: the editor has
// already selected the ids. It still enforces every publication guard that
// matters at write time so a stale, reported, out-of-window or out-of-catchment
// event cannot be frozen merely because its id remains in an old plan.
export async function weekendEventsByIds({ ids, lat, lng, radiusKm, from, to }) {
  if (!ids?.length) return [];
  await expireIfStale();
  const selected = ids.slice(0, 100);
  const rows = await sql`
    SELECT ${PUBLIC_EVENT_COLUMNS}, ri.n AS interest_count, rr.kind AS report_flag, hl.tier AS highlight
    FROM events e ${REACTION_JOIN} ${highlightJoin(from, to)}
    WHERE e.status='published' AND e.kind='event'
      AND rr.event_id IS NULL
      AND e.id IN ${sql(selected)}
      AND left(e.starts_at, 10) <= ${to}
      AND left(COALESCE(e.ends_at, e.starts_at), 10) >= ${from}
      AND ST_DWithin(e.geom::geography,
                     ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326)::geography,
                     ${radiusKm * 1000})
  `;
  return rows.map(shape).filter(communityQualityGate);
}

// Active newsletter recipients: double opt-in complete, not unsubscribed.
export async function confirmedSubscribers() {
  return await sql`
    SELECT id, email, source, lang, area_label, area_lat, area_lng, area_country,
           subscription_kind, channel_slug, radius_km, categories, token
    FROM subscribers
    WHERE confirmed_at IS NOT NULL AND unsubscribed_at IS NULL
    ORDER BY id
  `;
}

// Small key/value store (schema: `meta`). Used by the digest sender as a
// per-week send ledger so a re-run can never double-mail the list.
export async function metaGet(key) {
  const rows = await sql`SELECT value FROM meta WHERE key=${key}`;
  return rows[0]?.value ?? null;
}

// Every frozen weekend digest we have ever published, newest first. Each row is
// one shareable/indexable page (app/weekend/[city]/[weekend]) — the snapshot IS
// the archive, so nothing extra had to be stored to get one.
export async function listDigestKeys() {
  const rows = await sql`SELECT key FROM meta WHERE key LIKE 'digest:%' ORDER BY key DESC`;
  return rows
    .map((r) => r.key.split(':')) // digest:<slug>:<friday>
    .filter((p) => p.length === 3)
    .map(([, slug, friday]) => ({ slug, friday }));
}
// Same rows as listDigestKeys(), plus their raw value and send/post ledger
// flags — for the admin Pages desk (app/api/admin/pages), which would
// otherwise be an N+1 of metaGet calls (one per snapshot, three more per
// snapshot for the ledgers). Two queries total regardless of how many
// cities × weeks exist: the snapshot blobs are the only heavy part here, so
// callers should parse only what they need (itemCount, subject, etc.), not
// hold every snapshot's full item list in memory longer than it takes to map.
export async function listDigestPages() {
  const digestRows = await sql`SELECT key, value FROM meta WHERE key LIKE 'digest:%' ORDER BY key DESC`;
  const pages = digestRows
    .map((r) => ({ parts: r.key.split(':'), value: r.value })) // digest:<slug>:<friday>
    .filter((r) => r.parts.length === 3)
    .map(({ parts: [, slug, friday], value }) => ({ slug, friday, value }));

  if (!pages.length) return [];

  // Exact keys, not a LIKE scan: the same key families also hold per-EVENT
  // ledger rows (itemPostedKey appends `:ev:<id>`) that this view doesn't want.
  const ledgerKeys = pages.flatMap(({ slug, friday }) => [
    `sent:digest:${slug}:${friday}`,
    `posted:ig:${slug}:${friday}`,
    `posted:fb:${slug}:${friday}`,
  ]);
  const ledgerRows = await sql`SELECT key, value FROM meta WHERE key = ANY(${ledgerKeys})`;
  const ledger = new Map(ledgerRows.map((r) => [r.key, r.value]));

  return pages.map((p) => ({
    ...p,
    sentAt: ledger.get(`sent:digest:${p.slug}:${p.friday}`) ?? null,
    postedIg: ledger.has(`posted:ig:${p.slug}:${p.friday}`),
    postedFb: ledger.has(`posted:fb:${p.slug}:${p.friday}`),
  }));
}

export async function metaSet(key, value) {
  await sql`
    INSERT INTO meta (key, value) VALUES (${key}, ${value})
    ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value
  `;
}

// Atomic claim: insert-if-absent, true only for the request that won the row.
// This is the difference between "check then act" (racy) and a real mutex —
// two concurrent publishers both pass a metaGet check, but only one of these
// INSERTs counts. Claims are released with metaDelete.
export async function metaClaim(key, value) {
  const result = await sql`
    INSERT INTO meta (key, value) VALUES (${key}, ${value})
    ON CONFLICT (key) DO NOTHING
  `;
  return result.count === 1;
}

export async function metaDelete(key) {
  await sql`DELETE FROM meta WHERE key=${key}`;
}

export async function getEvent(id) {
  const today = viennaNow().slice(0, 10);
  const rows = await sql`
    SELECT ${PUBLIC_EVENT_COLUMNS}, ri.n AS interest_count, rr.kind AS report_flag, hl.tier AS highlight
    FROM events e ${REACTION_JOIN} ${highlightJoin(today)}
    WHERE e.id=${id} AND e.status='published'
  `;
  return rows.length ? shape(rows[0]) : null;
}

// Detail-only discovery: unlike the map payload, related events must not inherit
// the active viewport/date/category filters. A person opening one Posthof event
// should still see the venue's other upcoming programme. Name matching stays
// town-scoped; coordinate matching is allowed only for two precise locations so
// town-centroid sentinels can never merge a whole city.
export async function relatedUpcomingEvents(event, { limit = 12 } = {}) {
  if (!event) return { series: [], venue: [] };
  await expireIfStale();

  const cappedLimit = Math.max(1, Math.min(20, Number(limit) || 12));
  const today = viennaNow().slice(0, 10);
  const namedMatch = event.venue && event.town
    ? sql`(
        lower(trim(e.town)) = lower(trim(${event.town}))
        AND (
          lower(trim(e.venue)) = lower(trim(${event.venue}))
          OR (
            char_length(trim(${event.venue})) >= 6
            AND position(' ' || lower(trim(${event.venue})) || ' ' in ' ' || lower(trim(e.venue)) || ' ') > 0
          )
          OR (
            char_length(trim(e.venue)) >= 6
            AND position(' ' || lower(trim(e.venue)) || ' ' in ' ' || lower(trim(${event.venue})) || ' ') > 0
          )
        )
      )`
    : sql`false`;
  const precise = event.geo_precision && event.geo_precision !== 'town'
    && Number.isFinite(Number(event.lat)) && Number.isFinite(Number(event.lng));
  const point = precise
    ? sql`ST_SetSRID(ST_MakePoint(${Number(event.lng)}, ${Number(event.lat)}), 4326)`
    : null;
  const coordinateMatch = precise
    ? sql`(e.geo_precision <> 'town' AND ST_DWithin(e.geom::geography, ${point}::geography, 30))`
    : sql`false`;
  const venueMatch = sql`(${namedMatch} OR ${coordinateMatch})`;

  const seriesRows = event.kind === 'event' ? await sql`
      SELECT ${PUBLIC_EVENT_COLUMNS}, ri.n AS interest_count, rr.kind AS report_flag, hl.tier AS highlight
      FROM events e ${REACTION_JOIN} ${highlightJoin(today)}
      WHERE e.status='published' AND e.kind='event' AND e.id <> ${event.id}
        AND ${venueMatch}
        AND lower(trim(e.title)) = lower(trim(${event.title}))
      ORDER BY e.starts_at ASC
      LIMIT ${cappedLimit}
    ` : [];
  const venueRows = await sql`
    WITH distinct_titles AS (
      SELECT DISTINCT ON (lower(trim(e.title)))
             ${PUBLIC_EVENT_COLUMNS}, ri.n AS interest_count, rr.kind AS report_flag, hl.tier AS highlight
      FROM events e ${REACTION_JOIN} ${highlightJoin(today)}
      WHERE e.status='published' AND e.kind='event' AND e.id <> ${event.id}
        AND ${venueMatch}
        ${event.kind === 'event' ? sql`AND lower(trim(e.title)) <> lower(trim(${event.title}))` : sql``}
      ORDER BY lower(trim(e.title)), e.starts_at ASC, e.id ASC
    )
    SELECT * FROM distinct_titles
    ORDER BY starts_at ASC
    LIMIT ${cappedLimit}
  `;
  return { series: seriesRows.map(shape), venue: venueRows.map(shape) };
}

// SEO/event-detail landing pages are permanent URLs. An event that naturally
// finished remains readable as an archive, while deliberately removed rows
// stay private/404. Keep this separate from getEvent(): the map API uses that
// published-only read and must never resurrect an expired pin.
export async function getEventLanding(id) {
  if (!isEventId(id)) return null;
  await expireIfStale();
  const today = viennaNow().slice(0, 10);
  const rows = await sql`
    SELECT ${PUBLIC_EVENT_COLUMNS}, ri.n AS interest_count, rr.kind AS report_flag, hl.tier AS highlight
    FROM events e ${REACTION_JOIN} ${highlightJoin(today)}
    WHERE e.id=${id}
      AND (e.status='published' OR (e.status='expired' AND e.kind='event'))
  `;
  return rows.length ? shape(rows[0]) : null;
}

function recommendationKey(event) {
  const normalize = (value) => String(value || '')
    .normalize('NFKD')
    .replace(/\p{Diacritic}/gu, '')
    .toLocaleLowerCase()
    .replace(/[^\p{L}\p{N}]+/gu, ' ')
    .trim();
  return `${normalize(event.title)}|${String(event.starts_at || '').slice(0, 10)}|${normalize(event.town)}`;
}

// An event landing should lead into local discovery without loading the catalog
// into memory. Suggestions are future-starting events within a bounded radius.
// Family fit and shared categories rank first, then proximity/date. Expiry uses
// each event's IANA timezone, matching expireFinished(), never the host's.
export async function nearbyUpcomingEvents({ lat, lng, excludeId, categories = [], radiusKm = 40, limit = 3 } = {}) {
  await expireIfStale();
  const eventLat = Number(lat);
  const eventLng = Number(lng);
  if (!Number.isFinite(eventLat) || !Number.isFinite(eventLng)) return [];

  const cappedRadius = Math.max(1, Math.min(100, Number(radiusKm) || 40));
  const cappedLimit = Math.max(1, Math.min(6, Number(limit) || 3));
  const wantedCategories = Array.isArray(categories)
    ? categories.filter((category) => typeof category === 'string').slice(0, 12)
    : [];
  const point = sql`ST_SetSRID(ST_MakePoint(${eventLng}, ${eventLat}), 4326)`;
  const rows = await sql`
    SELECT e.id,e.kind,e.title,e.starts_at,e.ends_at,e.all_day,e.lat,e.lng,e.geo_precision,
           e.venue,e.address,e.town,e.country,e.categories,e.is_free,e.age_min,e.age_max,e.indoor,
           round((ST_Distance(e.geom::geography, ${point}::geography) / 1000)::numeric, 1)::float8
             AS distance_km
    FROM events e ${REACTION_JOIN}
    WHERE e.status='published' AND e.kind='event'
      ${excludeId ? sql`AND e.id <> ${excludeId}` : sql``}
      AND rr.event_id IS NULL
      -- "Coming up" must have a start date today or later in the EVENT's own
      -- timezone. A year-long course with a future ends_at is still published,
      -- but recommending its January start in August reads as plainly stale.
      -- CASE is required: Postgres may reorder boolean predicates, so a bare
      -- pg_input_is_valid() guard does not make a neighboring cast safe.
      AND CASE WHEN pg_input_is_valid(e.starts_at, 'timestamp') THEN
        e.starts_at::timestamp::date >= (
          now() AT TIME ZONE COALESCE(
            e.tz,
            CASE e.country
              WHEN 'AT' THEN 'Europe/Vienna'
              WHEN 'BG' THEN 'Europe/Sofia'
              WHEN 'DE' THEN 'Europe/Berlin'
              ELSE 'UTC'
            END
          )
        )::date
      ELSE false END
      -- Cheap GiST-backed bounding-box rejection before the exact geography
      -- distance calculation. At whole-catalog scale this is what keeps an
      -- indexed local recommendation from becoming a scan.
      AND e.geom && ST_Expand(${point}, ${cappedRadius / 111})
      AND ST_DWithin(e.geom::geography, ${point}::geography, ${cappedRadius * 1000})
      AND (
        COALESCE(
          CASE WHEN pg_input_is_valid(e.ends_at, 'timestamp') THEN
            CASE WHEN length(e.ends_at) = 10
                 THEN date_trunc('day', e.ends_at::timestamp) + interval '1 day'
                 ELSE e.ends_at::timestamp END
          END,
          CASE WHEN pg_input_is_valid(e.starts_at, 'timestamp') THEN
            CASE WHEN e.all_day OR length(e.starts_at) = 10
                 THEN date_trunc('day', e.starts_at::timestamp) + interval '1 day'
                 ELSE e.starts_at::timestamp + interval '6 hours' END
          END
        ) AT TIME ZONE COALESCE(
          e.tz,
          CASE e.country
            WHEN 'AT' THEN 'Europe/Vienna'
            WHEN 'BG' THEN 'Europe/Sofia'
            WHEN 'DE' THEN 'Europe/Berlin'
            ELSE 'UTC'
          END
        )
      ) >= now()
    ORDER BY (e.age_min IS NOT NULL OR 'family' = ANY(e.categories)) DESC,
             ${wantedCategories.length ? sql`(e.categories && ${wantedCategories}) DESC,` : sql``}
             distance_km ASC, e.starts_at ASC
    -- Pull a small lookahead so syndicated duplicates cannot consume all three
    -- user-visible cards; deduplication below still returns at most cappedLimit.
    LIMIT ${cappedLimit * 4}
  `;
  const unique = [];
  const seen = new Set();
  for (const row of rows) {
    const event = shape(row);
    const key = recommendationKey(event);
    if (seen.has(key)) continue;
    seen.add(key);
    unique.push(event);
    if (unique.length >= cappedLimit) break;
  }
  return unique;
}

export async function listSources() {
  return await sql`SELECT name,url,kind,town,works,last_crawled FROM sources ORDER BY name`;
}

function hashPart(s) {
  return (s || '').toLowerCase().replace(/[^a-z0-9äöüßЀ-ӿ]/g, '');
}

export function legacyContentHash(ev) {
  return `${hashPart(ev.title)}|${(ev.starts_at || '').slice(0, 10)}|${hashPart(ev.town)}`;
}

export function contentHash(ev) {
  // Charset extended for Cyrillic (Bulgaria) — Ѐ-ӿ is the full Cyrillic block
  // (U+0400-U+04FF). Adding characters to the allow-list never changes what
  // an existing German/Latin title normalizes to, so every hash already in
  // the DB stays byte-identical (verified: see the Developer-agent report for
  // this change — 5 sample AT/DE titles hashed before/after and matched).
  // Places dedup by title+town only (no date component — they're evergreen).
  if (ev.kind === 'place') return `place|${hashPart(ev.title)}|${hashPart(ev.town)}`;
  // One event can have multiple independently navigable occurrences on the
  // same day. Time and venue keep those rows distinct; fuzzy cross-source
  // dedup remains the layer for differently-worded copies of one occurrence.
  const starts = ev.starts_at || '';
  const location = ev.venue || ev.address || '';
  return `${hashPart(ev.title)}|${starts.slice(0, 10)}|${starts.slice(11, 16)}|${hashPart(ev.town)}|${hashPart(location)}`;
}

const bool = (v) => (v === null || v === undefined ? null : !!v);
const json = (v) => (v === null || v === undefined ? null : sql.json(v));

// age_min/age_max are int columns (db/schema.sql), but extraction (LLM/scrape)
// can hand back "4.5", "6+", or other non-integer junk. Coerce at the write
// boundary so every caller (crawl, seed, API POST, merge) is covered — never
// fabricate a value, only narrow what's already there. Non-finite -> null.
const toIntMin = (v) => {
  if (v === null || v === undefined || v === '') return null;
  const n = Number(v);
  return Number.isFinite(n) ? Math.floor(n) : null;
};
const toIntMax = (v) => {
  if (v === null || v === undefined || v === '') return null;
  const n = Number(v);
  return Number.isFinite(n) ? Math.ceil(n) : null;
};
const httpUrlOrNull = (value) => {
  if (!value) return null;
  try {
    const url = new URL(String(value));
    return /^https?:$/.test(url.protocol) ? url.toString() : null;
  } catch {
    return null;
  }
};

export async function upsertEvent(raw, { actorUserId = null } = {}) {
  // Entity decoding happens HERE, at the one write boundary every caller passes
  // through (crawl, seed, API POST, merge) — not in each adapter, where nine
  // partial copies had already drifted and let `&#8211;` reach 66 published
  // titles. Sources entity-encode inside JSON-LD/RSS/iCal, so a clean parser is
  // not enough; this is the same "coerce at the write boundary" rule as
  // toIntMin/toIntMax above. It runs BEFORE contentHash so the hash is computed
  // from the decoded title, not from the "8211" that hashPart() would keep.
  const ev = {
    ...raw,
    title: cleanText(raw.title),
    description: cleanText(raw.description),
    venue: cleanText(raw.venue),
    address: cleanText(raw.address),
    town: cleanText(raw.town),
    source_url: httpUrlOrNull(raw.source_url),
  };
  const kind = ev.kind === 'place' ? 'place' : 'event';
  if (kind === 'event') {
    if (!validLocalEventTime(ev.starts_at)) {
      const err = new Error(`Invalid event starts_at "${ev.starts_at ?? ''}"`);
      err.code = 'INVALID_EVENT_START';
      throw err;
    }
    // An invalid/garbled end is an unknown end, not a reason to reject a real
    // dated event. The same ends-after-starts invariant now lives at the final
    // write boundary, covering crawl, seed, scans, manual POSTs and migrations.
    if (ev.ends_at && (!validLocalEventTime(ev.ends_at) || ev.ends_at <= ev.starts_at)) {
      ev.ends_at = null;
    }
  }
  const hash = contentHash(ev);
  const cats = ev.categories || [];
  const ageMin = toIntMin(ev.age_min);
  const ageMax = toIntMax(ev.age_max);
  const tz = tzForEvent(ev);
  let existing = await sql`SELECT id,lat,lng FROM events WHERE content_hash=${hash}`;
  // Backward compatibility for rows written before occurrence-aware hashes:
  // lazily migrate the legacy title+day+town row only when its exact start and
  // non-conflicting venue match this occurrence. This avoids duplicating old
  // rows while never collapsing a second same-day performance into the first.
  if (!existing.length && kind === 'event') {
    const legacy = await sql`
      SELECT id,starts_at,venue,address,lat,lng FROM events WHERE content_hash=${legacyContentHash(ev)}
    `;
    const candidateLocation = hashPart(ev.venue || ev.address);
    const match = legacy.find((row) => (
      row.starts_at === ev.starts_at
      && (!hashPart(row.venue || row.address) || !candidateLocation
        || hashPart(row.venue || row.address) === candidateLocation)
    ));
    if (match) existing = [match];
  }
  // Placeholder-time migration: rows written before lib/event-time.js stored a
  // missing time as `T09:00` (+ all_day=true). This incoming event has NO time,
  // so its hash now ends in an empty time slot and would MISS that row — and
  // insert a second copy of an event we already have. Adopt the old row instead
  // and let the UPDATE below rewrite it to the honest date-only start.
  // Deliberately one-directional: an incoming event that DOES carry 09:00 takes
  // the normal path, so a genuine 9am event is never touched.
  if (!existing.length && kind === 'event' && ev.starts_at && !hasTime(ev.starts_at)) {
    const placeholder = contentHash({ ...ev, starts_at: `${ev.starts_at}T09:00` });
    const stale = await sql`SELECT id,lat,lng FROM events WHERE content_hash=${placeholder}`;
    if (stale.length) existing = [stale[0]];
  }
  if (existing.length) {
    const id = existing[0].id;
    // Crawler/seed callers retain the canonical upsert behavior. An account
    // submission may only update a row that account originally created; a
    // duplicate contribution to an official or another user's row is tracked
    // without mutating the canonical facts.
    if (actorUserId && !(await userCanEditContribution(actorUserId, id))) {
      return { id, updated: false, protected: true, lat: existing[0].lat, lng: existing[0].lng };
    }
    await sql`
      UPDATE events SET
        title=${ev.title}, emoji=${ev.emoji || null}, description=${ev.description || null},
        starts_at=${ev.starts_at || null}, ends_at=${ev.ends_at || null}, all_day=${!!ev.all_day},
        lat=${ev.lat}, lng=${ev.lng}, geo_precision=${ev.geo_precision || 'town'}, tz=${tz},
        venue=${ev.venue || null}, address=${ev.address || null}, categories=${cats},
        is_free=${bool(ev.is_free)}, age_min=${ageMin}, age_max=${ageMax},
        indoor=${bool(ev.indoor)}, source_url=${ev.source_url || null},
        opening_hours=${json(ev.opening_hours)}, seasonal=${ev.seasonal || null},
        content_hash=${hash}, updated_at=now()
      WHERE id=${id}
    `;
    return { id, updated: true };
  }
  const inserted = await sql`
    INSERT INTO events (kind,title,description,starts_at,ends_at,all_day,lat,lng,geo_precision,tz,
      venue,address,town,country,categories,is_free,age_min,age_max,indoor,emoji,photo_path,
      opening_hours,seasonal,status,src_kind,source_name,source_url,content_hash)
    VALUES (${kind},${ev.title},${ev.description || null},${ev.starts_at || null},${ev.ends_at || null},${!!ev.all_day},
      ${ev.lat},${ev.lng},${ev.geo_precision || 'town'},${tz},${ev.venue || null},${ev.address || null},
      ${ev.town || null},${ev.country || 'AT'},${cats},${bool(ev.is_free)},${ageMin},${ageMax},
      ${bool(ev.indoor)},${ev.emoji || null},${ev.photo_path || null},
      ${json(ev.opening_hours)},${ev.seasonal || null},${ev.status || 'published'},
      ${ev.src_kind || 'crawl'},${ev.source_name || null},${ev.source_url || null},${hash})
    RETURNING id
  `;
  return { id: inserted[0].id, updated: false };
}

export async function upsertSource(s) {
  await sql`
    INSERT INTO sources (name,url,kind,town,works,notes,cms,region,country)
    VALUES (${s.name},${s.url},${s.kind || 'municipal'},${s.town || null},${s.works === false ? false : true},${s.notes || null},${s.cms || null},${s.region || null},${s.country || 'AT'})
    ON CONFLICT (url) DO UPDATE SET works=EXCLUDED.works, notes=EXCLUDED.notes, cms=EXCLUDED.cms, region=EXCLUDED.region, country=EXCLUDED.country
  `;
}

// A licensed place import must not replace an earlier source's facts while
// retaining that source's attribution. Existing place identity is title+town.
export async function upsertCrawledPlace(place) {
  const hash = contentHash({ ...place, kind: 'place', title: cleanText(place.title), town: cleanText(place.town) });
  const [existing] = await sql`SELECT id, source_name FROM events WHERE content_hash=${hash}`;
  if (existing && existing.source_name !== place.source_name) {
    return { id: existing.id, updated: false, protected: true };
  }
  return upsertEvent({ ...place, kind: 'place' });
}

// --- geocache (used by lib/geocode.js) ---
export async function geocacheGet(q) {
  const rows = await sql`SELECT * FROM geocache WHERE query=${q}`;
  return rows[0] || null;
}
export async function geocacheSet(q, hit) {
  await sql`
    INSERT INTO geocache (query,lat,lng,label,hit)
    VALUES (${q},${hit?.lat ?? null},${hit?.lng ?? null},${hit?.label ?? null},${hit ? true : false})
    ON CONFLICT (query) DO UPDATE SET lat=EXCLUDED.lat, lng=EXCLUDED.lng, label=EXCLUDED.label, hit=EXCLUDED.hit
  `;
}
// Purge cached misses (hit=false). Whenever a rule feeding the geocache
// changes (bounds, waterfall order, POI matching), old negative entries can
// block results the new rule would have found — see tasks/lessons.md
// (2026-07-11, Bad Ischl). Cheap to recompute, poisonous to keep.
export async function purgeNegativeGeocache() {
  const res = await sql`DELETE FROM geocache WHERE hit=false`;
  return res.count;
}

// --- venues registry (docs/design/big-city-quality.md §1) ---
// Resolved (venue name, town) → coordinates, with provenance. Unlike the
// geocache (a query-string cache that gets purged when rules change), a venue
// row is a curated fact: once "Basilika St. Laurenz, Enns" is resolved, every
// current and future event at that venue geocodes instantly and identically.
export async function getVenue(nameNorm, townNorm, country = 'AT') {
  const rows = await sql`
    SELECT * FROM venues
    WHERE name_norm=${nameNorm} AND town_norm=${townNorm} AND country=${country}`;
  return rows[0] || null;
}

export function uniqueVenueRow(rows) {
  return Array.isArray(rows) && rows.length === 1 ? rows[0] : null;
}

// A source may call a known venue by a suburb/alias town (e.g. Linz-Urfahr)
// while the registry stores the municipality (Linz). Name-only lookup is safe
// only when that name is unique within the country; generic names such as
// "Pfarrheim" deliberately return null when they have multiple rows.
export async function getVenueByName(nameNorm, country = 'AT') {
  const rows = await sql`
    SELECT * FROM venues
    WHERE name_norm=${nameNorm} AND country=${country}
    LIMIT 2`;
  return uniqueVenueRow(rows);
}
// First resolution wins (ON CONFLICT DO NOTHING): a 'manual' or earlier fix is
// never clobbered by a later automated pass. Corrections go via direct SQL.
export async function upsertVenue(v) {
  await sql`
    INSERT INTO venues (name, town, country, name_norm, town_norm, lat, lng, geo_precision, resolved_via, source_url)
    VALUES (${v.name}, ${v.town ?? null}, ${v.country || 'AT'}, ${v.name_norm}, ${v.town_norm ?? ''},
            ${v.lat}, ${v.lng}, ${v.geo_precision || 'venue'}, ${v.resolved_via}, ${v.source_url ?? null})
    ON CONFLICT (name_norm, town_norm, country) DO NOTHING`;
}

// Moderation: flip an event's status ('published' | 'removed'). Deliberately
// NOT part of UPDATABLE_FIELDS — the dedup merge path must never touch status.
export async function setEventStatus(id, status) {
  const res = await sql`UPDATE events SET status=${status}, updated_at=now() WHERE id=${id} RETURNING id, title`;
  return res[0] || null;
}

// --- highlights (app/admin/highlights/page.js) ---
// Plain INSERT, never an upsert — a new period is a new row so history
// accrues (who was highlighted, when, why). setEventStatus-style validation
// (does the event exist / is it published) is the API route's job, via
// getEvent().
export async function setHighlight({ eventId, tier, startsAt, endsAt, note }) {
  const rows = await sql`
    INSERT INTO highlights (event_id, tier, starts_at, ends_at, note)
    VALUES (${eventId}, ${tier}, ${startsAt}, ${endsAt}, ${note || null})
    RETURNING id
  `;
  return rows[0].id;
}
export async function clearHighlight(id) {
  const res = await sql`DELETE FROM highlights WHERE id=${id}`;
  return res.count > 0;
}
// Every highlight period ever set, newest first, with enough of the event
// joined along to render a desk row without a second round-trip. `active`
// mirrors highlightJoin()'s own condition (period contains today-in-Vienna)
// plus the event still being published — a highlight on a removed/expired
// event is scheduling debris, not something live on the map right now.
export async function listHighlights() {
  const today = viennaNow().slice(0, 10);
  return await sql`
    SELECT h.id, h.event_id, h.tier, h.starts_at, h.ends_at, h.note, h.created_at,
           e.title AS event_title, e.town AS event_town, e.starts_at AS event_starts, e.status AS event_status,
           (h.starts_at <= ${today} AND h.ends_at >= ${today} AND e.status = 'published') AS active
    FROM highlights h
    JOIN events e ON e.id = h.event_id
    ORDER BY h.created_at DESC, h.id DESC
  `;
}

// ---- newsletter (double opt-in) ----
// Upsert a subscriber in PENDING state. An active subscriber's protected
// preferences and consent proof are left unchanged: knowing an email address
// must not be enough to move somebody between an edition and the waitlist.
// New / unconfirmed / previously unsubscribed rows get a fresh `token` and stay
// inactive until they confirm, so no one can (re)subscribe a stranger or
// resurrect an unsubscribe without proving control of the address. Every
// accepted pending signup also records proof of consent
// (Art. 7(1) GDPR): timestamp, the consent-wording version shown, hashed IP.
// Returns { pending } = a confirmation mail is due.
export async function addSubscriber(email, {
  source = null,
  lang = null,
  areaLabel = null,
  areaLat = null,
  areaLng = null,
  areaCountry = null,
  subscriptionKind = 'waitlist',
  channelSlug = null,
  radiusKm = 20,
  categories = [],
  token,
  consentVersion = null,
  consentIpHash = null,
} = {}) {
  const rows = await sql`
    INSERT INTO subscribers
      (email, source, lang, area_label, area_lat, area_lng, area_country, subscription_kind, channel_slug,
       radius_km, categories, token,
       token_issued_at, consent_version, consent_ip_hash, consent_at)
    VALUES
      (${email}, ${source}, ${lang}, ${areaLabel}, ${areaLat}, ${areaLng}, ${areaCountry}, ${subscriptionKind}, ${channelSlug},
       ${radiusKm}, ${categories}, ${token},
       now(), ${consentVersion}, ${consentIpHash}, now())
    ON CONFLICT (email) DO UPDATE SET
      source = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.source ELSE subscribers.source END,
      lang = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.lang ELSE subscribers.lang END,
      area_label = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.area_label ELSE subscribers.area_label END,
      area_lat = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.area_lat ELSE subscribers.area_lat END,
      area_lng = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.area_lng ELSE subscribers.area_lng END,
      area_country = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.area_country ELSE subscribers.area_country END,
      subscription_kind = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.subscription_kind ELSE subscribers.subscription_kind END,
      channel_slug = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.channel_slug ELSE subscribers.channel_slug END,
      radius_km = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.radius_km ELSE subscribers.radius_km END,
      categories = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.categories ELSE subscribers.categories END,
      consent_version = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.consent_version ELSE subscribers.consent_version END,
      consent_ip_hash = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN EXCLUDED.consent_ip_hash ELSE subscribers.consent_ip_hash END,
      consent_at = CASE WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL THEN now() ELSE subscribers.consent_at END,
      token = CASE
        WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL
        THEN EXCLUDED.token ELSE subscribers.token END,
      token_issued_at = CASE
        WHEN subscribers.confirmed_at IS NULL OR subscribers.unsubscribed_at IS NOT NULL
        THEN now() ELSE subscribers.token_issued_at END
    RETURNING token, (confirmed_at IS NULL OR unsubscribed_at IS NOT NULL) AS pending
  `;
  const row = rows[0] || {};
  return { pending: row.pending ?? true, token: row.token ?? token };
}

// Confirm links expire: ACTIVATING a subscription (first confirm, or reviving
// an unsubscribed one) requires a token issued within this window. Re-signing
// up always issues a fresh token, so an expired link is never a dead end. A
// re-click on an already-active subscription stays a harmless no-op at any age,
// and unsubscribe tokens deliberately never expire — revoking consent must
// always work (incl. RFC 8058 one-click). Mail copy mentions the 7 days too
// (CONFIRM_COPY in lib/mail.js) — keep them in sync.
const CONFIRM_TTL_DAYS = 7;

// Confirm the double opt-in: activate the subscriber and clear any prior
// unsubscribe. Idempotent — a second click keeps the original confirmed_at.
export async function confirmSubscriber(token) {
  if (!token) return null;
  const rows = await sql`
    WITH candidate AS (
      SELECT id, email, source, area_label, area_lat, area_lng, area_country,
             subscription_kind, channel_slug, lang,
             (confirmed_at IS NULL OR unsubscribed_at IS NOT NULL) AS newly_confirmed
      FROM subscribers
      WHERE token = ${token}
        AND (
          (confirmed_at IS NOT NULL AND unsubscribed_at IS NULL)
          OR token_issued_at IS NULL
          OR token_issued_at > now() - make_interval(days => ${CONFIRM_TTL_DAYS})
        )
      FOR UPDATE
    ), updated AS (
      UPDATE subscribers s
      SET confirmed_at = COALESCE(s.confirmed_at, now()), unsubscribed_at = NULL
      FROM candidate c
      WHERE s.id = c.id
      RETURNING s.id, s.email, s.source, s.area_label, s.area_lat, s.area_lng, s.area_country,
                s.subscription_kind, s.channel_slug, s.lang, c.newly_confirmed
    )
    SELECT * FROM updated
  `;
  if (!rows[0]) return null;
  return {
    id: String(rows[0].id),
    email: rows[0].email,
    source: rows[0].source,
    areaLabel: rows[0].area_label,
    areaLat: rows[0].area_lat,
    areaLng: rows[0].area_lng,
    areaCountry: rows[0].area_country,
    subscriptionKind: rows[0].subscription_kind,
    channelSlug: rows[0].channel_slug,
    lang: rows[0].lang,
    newlyConfirmed: rows[0].newly_confirmed,
  };
}

// Token-authenticated newsletter settings. The token is the same unguessable
// secret already used for unsubscribe; only confirmed, still-active rows may be
// read or changed. No email address is returned to the page.
export async function subscriberPreferences(token) {
  if (!token) return null;
  const rows = await sql`
    SELECT id, lang, area_label, area_lat, area_lng, area_country, subscription_kind, channel_slug
    FROM subscribers
    WHERE token=${token} AND confirmed_at IS NOT NULL AND unsubscribed_at IS NULL
  `;
  if (!rows[0]) return null;
  return {
    id: String(rows[0].id),
    lang: rows[0].lang,
    areaLabel: rows[0].area_label,
    areaLat: rows[0].area_lat,
    areaLng: rows[0].area_lng,
    areaCountry: rows[0].area_country,
    subscriptionKind: rows[0].subscription_kind,
    channelSlug: rows[0].channel_slug,
  };
}

export async function updateSubscriberPreferences(token, {
  areaLabel,
  areaLat,
  areaLng,
  areaCountry,
  subscriptionKind,
  channelSlug,
  consentVersion,
  consentIpHash,
} = {}) {
  if (!token) return false;
  const rows = await sql`
    UPDATE subscribers
    SET area_label=${areaLabel}, area_lat=${areaLat}, area_lng=${areaLng}, area_country=${areaCountry},
        subscription_kind=${subscriptionKind}, channel_slug=${channelSlug},
        consent_version=${consentVersion}, consent_ip_hash=${consentIpHash}, consent_at=now()
    WHERE token=${token} AND confirmed_at IS NOT NULL AND unsubscribed_at IS NULL
    RETURNING id
  `;
  return rows[0] ? String(rows[0].id) : null;
}

// One-click unsubscribe via the per-subscriber token. Idempotent.
export async function unsubscribe(token) {
  if (!token) return null;
  const rows = await sql`
    UPDATE subscribers
    SET unsubscribed_at = COALESCE(unsubscribed_at, now())
    WHERE token = ${token}
    RETURNING email
  `;
  return rows[0]?.email ?? null;
}

// ---- rate limiting (durable opaque principal hashes; never raw IPs/emails/UUIDs) ----
export async function recordRateHit(ipHash, action) {
  await sql`INSERT INTO rate_hits (ip_hash, action) VALUES (${ipHash}, ${action})`;
}
export async function countRateHits(ipHash, action, minutes) {
  const rows = await sql`
    SELECT count(*)::int AS n FROM rate_hits
    WHERE ip_hash=${ipHash} AND action=${action} AND at > now() - (${minutes} * interval '1 minute')
  `;
  return rows[0]?.n ?? 0;
}
export async function countActionAll(action, minutes) {
  const rows = await sql`
    SELECT count(*)::int AS n FROM rate_hits
    WHERE action=${action} AND at > now() - (${minutes} * interval '1 minute')
  `;
  return rows[0]?.n ?? 0;
}

// Atomically check and consume one durable rate-limit slot. The transaction
// locks the action-wide bucket before the principal bucket, so concurrent
// serverless requests cannot all observe the same pre-insert count and burst
// through a quota. The lock identifiers are hashes; no user identifier is
// persisted or exposed by the lock itself.
export async function takeRateSlot(principalHash, action, { perHour, perDay, globalPerDay } = {}) {
  return sql.begin(async (tx) => {
    if (globalPerDay != null) {
      await tx`SELECT pg_advisory_xact_lock(hashtextextended(${`rate-global|${action}`}, 0))`;
    }
    await tx`SELECT pg_advisory_xact_lock(hashtextextended(${`rate-principal|${action}|${principalHash}`}, 0))`;

    if (perHour != null) {
      const [row] = await tx`
        SELECT count(*)::int AS n FROM rate_hits
        WHERE ip_hash=${principalHash} AND action=${action} AND at > now() - interval '60 minutes'
      `;
      if ((row?.n ?? 0) >= perHour) return { retryAfter: 3600, scope: 'principal', window: 'hour', max: perHour };
    }
    if (perDay != null) {
      const [row] = await tx`
        SELECT count(*)::int AS n FROM rate_hits
        WHERE ip_hash=${principalHash} AND action=${action} AND at > now() - interval '1440 minutes'
      `;
      if ((row?.n ?? 0) >= perDay) return { retryAfter: 86400, scope: 'principal', window: 'day', max: perDay };
    }
    if (globalPerDay != null) {
      const [row] = await tx`
        SELECT count(*)::int AS n FROM rate_hits
        WHERE action=${action} AND at > now() - interval '1440 minutes'
      `;
      if ((row?.n ?? 0) >= globalPerDay) return { retryAfter: 86400, scope: 'global', window: 'day', max: globalPerDay };
    }
    await tx`INSERT INTO rate_hits (ip_hash, action) VALUES (${principalHash}, ${action})`;
    return null;
  });
}

// --- source crawl helpers (used by scripts/crawl.mjs) ---
export async function getSourceByUrl(url) {
  const rows = await sql`SELECT * FROM sources WHERE url=${url}`;
  return rows;
}
// Read-only, all sources regardless of works/tier — used by
// scripts/probe-sources.mjs to skip municipalities already registered
// (matched by domain or name+region), never to write.
export async function listSourcesForDedup() {
  return await sql`SELECT name, url, town, region FROM sources`;
}
export async function getWorkingSources() {
  return await sql`SELECT * FROM sources WHERE works=true`;
}
export async function markSourceCrawled(id) {
  await sql`UPDATE sources SET last_crawled=now() WHERE id=${id}`;
}
// page_hash: sha256 of the stripped page text, for change-detection skip.
// feed_kind: which extraction route won this crawl
// (jsonld|ical|gem2go|dvv|rss|venue-specific adapters|llm|null).
// etag/last_modified: the 200 response's caching headers (generic shell
// only), so the next crawl can send If-None-Match/If-Modified-Since and skip
// a re-download entirely on a 304 — cheaper than the page_hash compare, which
// still needs the full body. A 304 response never reaches this function (it
// takes the same early-return path as an unchanged page_hash, so the stored
// values are left untouched); callers with no HTTP-level etag concept
// (naturfreunde's POST-based special case) simply omit them, same as today.
export async function updateSourceMeta(id, { page_hash, feed_kind, etag, last_modified } = {}) {
  await sql`
    UPDATE sources SET page_hash=${page_hash ?? null}, feed_kind=${feed_kind ?? null},
      etag=${etag ?? null}, last_modified=${last_modified ?? null}
    WHERE id=${id}
  `;
}
export async function setSourceNote(id, note) {
  await sql`UPDATE sources SET notes=${note} WHERE id=${id}`;
}
// blocked_reason (docs/design/big-city-quality.md §2): a STATE ('robots' |
// 'ai_bot_policy' | 'js_spa' | 'bot_block'), not a failure streak — pass null
// to clear it once a fetch succeeds again. Deliberately separate from
// setSourceNote/updateSourceStats: setting/clearing this must never itself
// touch zero_streak or tier.
export async function setSourceBlockedReason(id, reason) {
  await sql`UPDATE sources SET blocked_reason=${reason} WHERE id=${id}`;
}

// --- source content-rating / tiering (used by scripts/crawl.mjs) ---
// Sources for a normal run: every working source, including tier='dead'.
// Cadence is applied by the caller; dead is a 28-day quarantine whose due
// attempts are forced through extraction so a false positive can revive.
// Order MOST-OVERDUE FIRST (never-crawled, then oldest last_crawled), not by id.
// A run that doesn't finish the full due-set — Actions cancellation/restart,
// timeout, a crash — always stopped at the SAME high-id tail under `ORDER BY id`,
// so ~500 sources sat un-fetched for days (267 stuck at their first-ever crawl,
// events_last=0, looking dead while being alive). Ordering by staleness makes a
// partial run spend its budget on whatever has waited longest instead of
// re-walking the same head every night. `groupByHost` in crawl.mjs still
// regroups for politeness, so this only changes WHICH sources get reached first.
export async function getSourcesForCrawl() {
  const order = sql`ORDER BY last_crawled ASC NULLS FIRST, id`;
  return await sql`SELECT * FROM sources WHERE works=true ${order}`;
}
// Recovery set for `crawl.mjs --recover-zeros`: sources frozen at zero yield.
// tier='dead' is deliberately INCLUDED — a provider-failure window can rot a
// healthy source's zero_streak to 4, and this pass is how it comes back.
export async function getZeroYieldSources() {
  return await sql`
    SELECT * FROM sources WHERE works=true AND COALESCE(events_last, 0) = 0
    ORDER BY last_crawled ASC NULLS FIRST, id`;
}

// --- CMS fingerprint sweep (scripts/fingerprint-sources.mjs) ---
// Target set: working sources still paying the LLM route, or never fetched
// with HTML (feed_kind null), or classified from URL-only heuristics that
// never confirmed a real markup marker (unknown/other/null cms). Read-only
// consumer; --write applies via updateSourceCmsFingerprint below.
export async function getFingerprintCandidates({ country } = {}) {
  return await sql`
    SELECT * FROM sources
    WHERE works=true
      AND (
        feed_kind = 'llm' OR feed_kind IS NULL
        OR cms IN ('unknown', 'other') OR cms IS NULL
      )
      ${country ? sql`AND country = ${country}` : sql``}
    ORDER BY id
  `;
}
// Applies a fingerprint sweep's proposed cms (only ever a value crawl.mjs
// routes — see lib/cms-fingerprint.js ROUTABLE_CMS) and/or an explanatory
// note. Never touches feed_kind/page_hash — those are the crawler's own
// bookkeeping, updated only by an actual crawl (updateSourceMeta).
export async function updateSourceCmsFingerprint(id, { cms, notes } = {}) {
  await sql`
    UPDATE sources SET
      cms = COALESCE(${cms ?? null}, cms),
      notes = COALESCE(${notes ?? null}, notes)
    WHERE id=${id}
  `;
}
// Caller (crawl.mjs) computes every field from the row it already read plus
// this run's outcome — see the tier-threshold comment there for the policy.
export async function updateSourceStats(id, { crawl_count, events_last, events_sum, zero_streak, last_changed, tier }) {
  await sql`
    UPDATE sources SET
      crawl_count=${crawl_count}, events_last=${events_last}, events_sum=${events_sum},
      zero_streak=${zero_streak}, last_changed=${last_changed ?? null}, tier=${tier}
    WHERE id=${id}
  `;
}

// --- geocode repair (used by scripts/regeocode.mjs) ---
// Rows worth re-checking: already at town-centroid precision (nothing more
// precise was ever found), or carrying a venue name (candidate for the
// POI-first waterfall to do better than whatever originally resolved it).
export async function getGeocodeCandidateRows() {
  return await sql`
    SELECT id, kind, title, venue, address, town, lat, lng, geo_precision
    FROM events
    WHERE geo_precision = 'town' OR venue IS NOT NULL
    ORDER BY id
  `;
}
export async function updateEventCoords(id, { lat, lng, geo_precision }) {
  await sql`UPDATE events SET lat=${lat}, lng=${lng}, geo_precision=${geo_precision}, updated_at=now() WHERE id=${id}`;
}

// --- crawl-time fuzzy dedup (scripts/crawl.mjs, lib/dedup.js's findDuplicate) ---
// Bounded candidate set for a single new event: same Vienna calendar day + same
// town/precise location, excluding the row just written when applicable.
// app/api/events/route.js, app/api/scan/route.js, and crawl.mjs all use this
// scoped query instead of loading every published event — the crawl calls it once per
// freshly-inserted row, and a full-table load per candidate would multiply the
// cost of an already-large crawl by the size of the whole events table.
export async function dedupCandidates(day, town, excludeId, {
  lat, lng, geo_precision: geoPrecision,
} = {}) {
  const hasTown = typeof town === 'string' && town.trim() !== '';
  const hasPreciseCoords = (
    geoPrecision && geoPrecision !== 'town'
    && Number.isFinite(Number(lat)) && Number.isFinite(Number(lng))
  );
  if (!day || (!hasTown && !hasPreciseCoords)) return [];
  const point = hasPreciseCoords
    ? sql`ST_SetSRID(ST_MakePoint(${Number(lng)}, ${Number(lat)}), 4326)`
    : null;
  const townMatch = hasTown ? sql`lower(e.town) = lower(${town})` : sql`false`;
  const coordMatch = hasPreciseCoords ? sql`
    e.geo_precision <> 'town'
    AND e.geom && ST_Expand(${point}, ${0.3 / 111})
    AND ST_DWithin(e.geom::geography, ${point}::geography, 300)
  ` : sql`false`;
  const rows = await sql`
    SELECT e.id, e.kind, e.title, e.starts_at, e.ends_at, e.all_day, e.town, e.lat, e.lng,
           e.geo_precision, e.description, e.address, e.venue, e.is_free, e.age_min, e.age_max,
           e.indoor, e.photo_path, e.categories
    FROM events e
    WHERE e.status='published' AND e.kind='event'
      AND left(e.starts_at, 10) = ${day}
      AND (${townMatch} OR (${coordMatch}))
      ${excludeId ? sql`AND e.id <> ${excludeId}` : sql``}
  `;
  return rows.map(shape);
}

// Scripts must close the pool so the Node process can exit.
export async function closeDb() {
  await sql.end({ timeout: 5 });
}

// --- dedup merge (used by app/api/events/route.js, app/api/scan/route.js,
// scripts/merge-dups.mjs via lib/dedup.js's mergePlan) ---
// Parameterized UPDATE over a whitelisted field set — patch is the output of
// mergePlan(), never raw request input, so the whitelist is a belt-and-braces
// guard rather than the only line of defense.
const UPDATABLE_FIELDS = new Set([
  'description', 'ends_at', 'address', 'venue', 'is_free',
  'age_min', 'age_max', 'indoor', 'photo_path', 'categories',
]);
const OWNED_UPDATABLE_FIELDS = new Set([
  ...UPDATABLE_FIELDS, 'lat', 'lng', 'geo_precision',
]);
export async function updateEventFields(id, patch) {
  const entries = Object.entries(patch || {}).filter(([k]) => UPDATABLE_FIELDS.has(k));
  if (!entries.length) return { id, updated: false };
  const set = {};
  for (const [k, v] of entries) {
    set[k] = k === 'is_free' || k === 'indoor' ? bool(v) : v;
  }
  await sql`UPDATE events SET ${sql(set)}, updated_at=now() WHERE id=${id}`;
  return { id, updated: true };
}

export async function updateOwnedEventFields(userId, id, patch) {
  if (!(await userCanEditContribution(userId, id))) return { id, updated: false, protected: true };
  const entries = Object.entries(patch || {}).filter(([k]) => OWNED_UPDATABLE_FIELDS.has(k));
  if (!entries.length) return { id, updated: false, protected: false };
  const set = {};
  for (const [k, v] of entries) {
    set[k] = k === 'is_free' || k === 'indoor' ? bool(v) : v;
  }
  await sql`UPDATE events SET ${sql(set)}, updated_at=now() WHERE id=${id}`;
  return { id, updated: true, protected: false };
}
// Cross-source dedup cleanup (scripts/merge-dups.mjs --write): remove the
// newer duplicate rows once their fields have been merged onto the canonical
// (oldest) row via updateEventFields/mergePlan above.
export async function deleteEventsByIds(ids) {
  if (!ids || !ids.length) return 0;
  const res = await sql`DELETE FROM events WHERE id IN ${sql(ids)}`;
  return res.count;
}
