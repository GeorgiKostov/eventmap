...
export async function seoEvents({ lat, lng, radiusKm, from, to, kids = false, limit = 80 }) {
  await expireIfStale();
  const pageLimit = Math.max(1, Math.min(100, Number(limit) || 80));
  const point = sql`ST_SetSRID(ST_MakePoint(${lng}, ${lat}), 4326)`;
  const rows = await sql`
    WITH matched AS (
      SELECT e.id,e.title,e.starts_at,e.all_day,e.venue,e.town,e.categories,e.is_free,
             row_number() OVER (
               PARTITION BY lower(e.title), COALESCE(NULLIF(lower(e.venue), ''), lower(e.town), '')
               ORDER BY (left(e.starts_at, 10) < ${from}), e.starts_at
             ) AS series_rank
      FROM events e ${REACTION_JOIN}
      WHERE e.status='published' AND e.kind='event'
    ...
    SELECT id,title,starts_at,all_day,venue,town,categories,is_free,
           count(*) OVER()::int AS total_count
    FROM matched
    WHERE series_rank = 1
    ORDER BY (left(starts_at, 10) < ${from}), starts_at
    LIMIT ${pageLimit}
  `;
  const total = rows.length ? Number(rows[0].total_count) : 0;
  return {
  ...
  const rows = await sql`
    WITH windows AS (
      SELECT * FROM (VALUES ${sql(cityWindows.map(({ city, window }) => [
        ...
        window.to,
        Boolean(window.kids),
      ]))}) AS w(city_slug, lat, lng, radius_km, window_key, date_from, date_to, kids)
    ), reports AS (
      SELECT event_id
      FROM reactions
      WHERE kind <> 'interest'
    ...
    SELECT w.city_slug, w.window_key,
           count(e.id) FILTER (WHERE r.event_id IS NULL)::int AS event_count,
    ...
    LEFT JOIN events e ON e.status='published' AND e.kind='event'
      AND left(e.starts_at, 10) <= w.date_to
