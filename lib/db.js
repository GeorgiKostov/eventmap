...
export async function upsertEvent(ev) {
  ...
  if (existing.length) {
    ...
    await sql`
      UPDATE events SET
        ...
        starts_at=${ev.starts_at}, ends_at=${ev.ends_at || null}, all_day=${!!ev.all_day},
        ...
        indoor=${bool(ev.indoor)}, source_url=${ev.source_url || null}, updated_at=now()
      WHERE id=${id}
    `;
  ...
  const inserted = await sql`
    ...
    VALUES (${ev.title},${ev.description || null},${ev.starts_at},${ev.ends_at || null},${!!ev.all_day},
      ...
      ${bool(ev.indoor)},${ev.emoji || null},${ev.photo_path || null},${ev.status || 'published'},
...
export async function upsertSource(s) {
  await sql`
    INSERT INTO sources (name,url,kind,town,works,notes)
    VALUES (${s.name},${s.url},${s.kind || 'municipal'},${s.town || null},${s.works === false ? false : true},${s.notes || null})
    ON CONFLICT (url) DO UPDATE SET works=EXCLUDED.works, notes=EXCLUDED.notes
  `;
}
