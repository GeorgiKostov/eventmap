...
// "Community" = user-submitted content (one closed set, used by both the
// map's Community filter and the digest ranking). NOT "anything not crawled" —
// bulk osm_mined places are not community submissions.
const COMMUNITY_KINDS = new Set(['user_photo', 'user_manual', 'user_link']);
...
export async function weekendPicks({ lat, lng, radiusKm, from, to, limit = 10 }) {
  ...
  `;
  // Ranking is LEXICOGRAPHIC, not an additive sum: an additive sum let three
  // lower signals (free+community+precise = 5) outrank the product lens itself
  // (family = 4), so a non-family event could headline the family digest. As a
  // tuple, family fit is strictly dominant, then free, then community-submitted,
  // then precise location, then soonest. "Community" is user-submitted content —
  // the same closed set commonFilters uses — NOT merely "not crawled" (which also
  // caught bulk osm_mined places).
  const rank = (e) => [
    isForKids(e) ? 1 : 0,
    e.is_free ? 1 : 0,
    COMMUNITY_KINDS.has(e.src_kind) ? 1 : 0,
