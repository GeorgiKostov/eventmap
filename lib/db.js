...
// `highlights`): picks the single strongest currently-active period for an
// event — gold outranks editorial — so a row carries at most one tier.
// `today` is the caller's viennaNow().slice(0,10) (wall-clock, never
// host/UTC, per CLAUDE.md rule 3).
function highlightJoin(today) {
  return sql`
    LEFT JOIN LATERAL (
      SELECT h.tier FROM highlights h
      WHERE h.event_id = e.id AND h.starts_at <= ${today} AND h.ends_at >= ${today}
