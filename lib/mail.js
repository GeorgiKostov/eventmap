...
const PREFERENCES_COPY = {
  de: {
    ...
    line: 'Bitte wähle deine Okolo-Ausgabe. Wenn deine Stadt noch nicht dabei ist, benachrichtigen wir dich auf Wunsch einmalig zum Start.',
  ...
  en: {
    ...
    line: 'Please choose your Okolo edition. If your city is not available yet, we can notify you once when it launches.',
  ...
  bg: {
    ...
    line: 'Избери своето издание на Okolo. Ако твоят град още не е наличен, можем да те уведомим еднократно при старта.',
...
const LAUNCH_COPY = {
  de: {
    subject: (city) => `Okolo ${city} ist da`,
    line: (city) => `Die wöchentliche Okolo-Ausgabe für ${city} & Umgebung ist jetzt verfügbar. Wähle sie aus, um donnerstags die Wochenend-Tipps zu bekommen.`,
    cta: 'Newsletter auswählen', why: 'Du hast um eine einmalige Nachricht gebeten, sobald Okolo in deiner Stadt startet.', unsub: 'Abmelden',
  },
  en: {
    subject: (city) => `Okolo ${city} is live`,
    line: (city) => `The weekly Okolo edition for ${city} & surroundings is now available. Select it to receive weekend tips every Thursday.`,
    cta: 'Choose newsletter', why: 'You asked us to notify you once when Okolo launched in your city.', unsub: 'Unsubscribe',
  },
  bg: {
    subject: (city) => `Okolo ${city} стартира`,
    line: (city) => `Седмичното издание на Okolo за ${city} и околността вече е налично. Избери го, за да получаваш идеи за уикенда всеки четвъртък.`,
    cta: 'Избери бюлетина', why: 'Поиска да те уведомим еднократно, когато Okolo стартира в твоя град.', unsub: 'Отписване',
  },
};

export async function sendNewsletterLaunchNotice({ to, lang, city, preferencesUrl, unsubscribeUrl, idempotencyKey }) {
  const c = LAUNCH_COPY[lang] || LAUNCH_COPY.en;
  return deliver({
    to,
    subject: c.subject(city),
    text: `${c.line(city)}\n\n${c.cta}: ${preferencesUrl}\n\n${c.why}\n${c.unsub}: ${unsubscribeUrl}`,
    html: `<p>${c.line(city)}</p><p><a href="${preferencesUrl}" style="display:inline-block;padding:11px 18px;background:#C93A5B;color:#fff;border-radius:8px;text-decoration:none;font-weight:700">${c.cta}</a></p><p style="color:#667;font-size:13px">${c.why}<br><a href="${unsubscribeUrl}" style="color:#667">${c.unsub}</a></p>`,
    idempotencyKey,
    headers: {
      'List-Unsubscribe': `<${unsubscribeUrl}>`,
      'List-Unsubscribe-Post': 'List-Unsubscribe=One-Click',
