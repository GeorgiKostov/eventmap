...
import { geocacheGet, geocacheSet, COUNTRY_TZ } from './db.js';
...
async function nominatimSearch(q, { limit = 1, country = 'AT' } = {}) {
  ...
  const url =
    `https://nominatim.openstreetmap.org/search?format=json&limit=${limit}&countrycodes=${codes}&namedetails=1&addressdetails=1&q=` +
...
// supply backfill; still rejects same-named places abroad (DE/CH/IT). BG box
// added for the Bulgaria mining pass (docs/playbooks/country-mining-playbook.md).
...
export async function geocodeEvent(ev) {
  ...
  const suffix = countryMeta(country).suffix;
  if (ev.venue) {
    const hit = await poiQuery(ev.venue, ev.town, country);
  ...
  if (ev.venue) {
    const hit = await tryQuery(`${ev.venue}, ${ev.town || ''}, ${suffix}`, country);
    if (hit) return { ...hit, geo_precision: 'venue' };
  }
