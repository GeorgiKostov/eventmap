...
import { claimExternalRateSlot, geocacheGet, geocacheSet, getVenue, getVenueByName, upsertVenue, COUNTRY_TZ } from './db.js';
...
const UA = 'Okolo/1.0 (+https://www.okolo.events; contact: hello@okolo.events)';
let lastCall = 0;
...
function throttle() {
  ...
  const mine = throttleChain.then(async () => {
    // Fast local guard avoids a database round trip for calls already queued in
    // this warm instance. The durable claim then enforces the same interval
    // across every Vercel instance, which an in-memory timestamp cannot do.
    const localWait = 1100 - (Date.now() - lastCall);
    if (localWait > 0) await new Promise((r) => setTimeout(r, localWait));
    while (true) {
      const sharedWait = await claimExternalRateSlot('nominatim-public', 1100);
      if (sharedWait <= 0) break;
      await new Promise((r) => setTimeout(r, sharedWait));
    }
    lastCall = Date.now();
  });
...
export function registryVenueNearTown(hit, expected) {
  return !!hit && !!expected && distanceKm(hit, expected) <= TOWN_BOUND_KM;
}
...
// throwOnError: re-throw a transient failure instead of swallowing it to a
// plain miss. Only poiQuery's internal "locate the expected town" lookup uses
// this — it needs to tell "town genuinely not found" (cache negative, as
// always) apart from "couldn't check right now" (a 429/5xx/timeout on THIS
// sub-lookup), because the caller was about to cache a DIFFERENT key (the poi:
// query) based on the answer. Every other caller keeps the old behavior.
async function tryQuery(q, country = 'AT', { throwOnError = false } = {}) {
...
async function poiQuery(venue, town, country = 'AT') {
  ...
  if (!expected && town) {
    try {
      expected = await tryQuery(`${town}, ${suffix}`, country, { throwOnError: true });
    } catch {
      // A transient failure (429/5xx/timeout) locating the expected town is
      // NOT the same as "town genuinely doesn't exist" — caching `null` here
      // would poison THIS poi query's cache entry over an unrelated lookup's
      // rate-limit storm (the exact bug class this file already guards
      // against elsewhere). Miss without recording anything; retried next time.
      return null;
    }
  }
  if (!expected && town) {
    // Town couldn't be located at all (a genuine, confirmed miss) → can't
    // verify "near the expected town", so don't risk an unbounded POI match.
    // Fall through to the weaker (but at-least-not-worse) address/plain-venue/
    // town fallbacks.
...
export async function geocodeEvent(ev, { jitterTown = true } = {}) {
  ...
  // A precise hit must land NEAR THE EXPECTED TOWN. Generic names ("Gemeindeamt",
  // "Bühne 3") exist in every Austrian town, so an unbounded match can be placed
  // hundreds of km away at full venue precision — strictly worse than an honest
  // town centroid. Applies to the registry rung too: migrate-venues.mjs seeded
  // it from pre-bound-fix events (254 rows up to 446 km off, tasks/lessons.md),
  // and the registry short-circuits every later check, so if it isn't bounded a
  // poisoned row is served forever. Town-precision registry rows are the centroid
  // by definition, so they can't violate the bound.
  const nearTown = async (hit) => {
    ...
    return registryVenueNearTown(hit, expected);
  };
  if (venue) {
    // Registry first: a previously resolved (venue, town) is a fact, cheaper
    // and more trustworthy than re-deriving it from Nominatim.
    const exactReg = await getVenue(normalizeName(venue), normalizeName(ev.town || ''), country);
    if (exactReg && (exactReg.geo_precision === 'town' || await nearTown(exactReg))) {
      return { lat: exactReg.lat, lng: exactReg.lng, label: exactReg.name, geo_precision: exactReg.geo_precision };
    }
    // Preserve the exact town key as the primary lookup. If a source uses a
    // suburb/alias town, accept a same-country name-only registry row only
    // when it is unique; the nearTown guard also applies to this fallback.
    const fallbackReg = !exactReg ? await getVenueByName(normalizeName(venue), country) : null;
    if (fallbackReg && await nearTown(fallbackReg)) {
      return { lat: fallbackReg.lat, lng: fallbackReg.lng, label: fallbackReg.name, geo_precision: fallbackReg.geo_precision };
    }
  ...
  // The plain address / venue+town Nominatim rungs use the same town bound
  // (nearTown, defined above) — an unbounded match here put "Bühne 3", a stage
  // inside a Vienna children's theatre, 24 km outside Vienna.
  if (ev.address) {
  ...
  // NO JITTER. A town-level result means "somewhere in this town" — scattering
  // it ±300m to stop pins stacking INVENTED a position for every one of ~11k
  // events (170 of them in Wiener Neustadt alone, spread across a few blocks at
  // random coordinates that looked exactly like real venues at street zoom).
  // That is fabrication in service of a tidy map, and hard rule 5 has no
  // cosmetic exemption. Every town-level event now sits exactly on the town's
  // centroid, and the map collapses them into ONE honest "N events in <town>"
  // group (a dashed bubble — approximate — rather than a pin, which claims a
  // venue). The `jitterTown` option is kept only so callers can't break, but it
  // no longer moves anything. (2026-07-14, George: "how do we communicate what
  // it is exactly so it's not confusing".)
  ...
  if (c) return { lat: c.lat, lng: c.lng, geo_precision: 'town' };
  if (ev.town) {
    const hit = await tryQuery(`${ev.town}, ${suffix}`, country);
    if (hit) return { lat: hit.lat, lng: hit.lng, geo_precision: 'town' };
  }
  return null;
}
...
export async function reverseGeocode(lat, lng) {
  ...
  try {
    ...
    const res = await fetch(url, { headers: { 'User-Agent': UA }, signal: AbortSignal.timeout(15000) });
    // Transient (rate-limit / server) → return null WITHOUT caching, same fix
    // as reverseGeocodeAddress/tryQuery: this branch used to fall through with
    // label still null and cache THAT as a genuine miss, silently freezing a
    // 429 into "no locality here" forever.
