...
// A same-title cluster is a real "series" only when its occurrences share a
// venue. When the same title runs at MULTIPLE distinct named venues in a town
// (e.g. a workshop held at five community centres), collapsing them to one pin
// would erase four real locations — so split by venue name. Members with no
// venue name stand alone rather than being guessed into one of the venues.
function splitByVenue(members) {
  const labels = new Set();
  for (const ev of members) {
    const v = normalizedLabel(ev.venue);
    if (v) labels.add(v);
  }
  if (labels.size <= 1) return [members];
  const byVenue = new Map();
  const singles = [];
  for (const ev of members) {
    const v = normalizedLabel(ev.venue);
    if (!v) { singles.push([ev]); continue; }
    if (!byVenue.has(v)) byVenue.set(v, []);
    byVenue.get(v).push(ev);
  }
  return [...byVenue.values(), ...singles];
}
...
export function groupEventSeries(events) {
  ...
  for (const townEvents of byTown.values()) {
    ...
    for (const group of townGroups) {
      for (const sub of splitByVenue(group.members)) {
        if (sub.length < 2) continue;
        const series = { members: sub, anchor: anchorFor(sub) };
        groups.push(series);
        for (const event of sub) byId.set(event.id, series);
      }
    }
  }
  return { groups, byId };
