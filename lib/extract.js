...
let meteredTextCalls = 0;

class ExtractionBudgetError extends Error {
  constructor(limit) {
    super(`LLM call budget exhausted (${limit} metered text request(s))`);
    this.name = 'ExtractionBudgetError';
    this.code = 'EXTRACTION_BUDGET_EXHAUSTED';
  }
}

function claimMeteredTextCall() {
  const raw = process.env.MAX_LLM_CALLS;
  if (!raw) {
    meteredTextCalls++;
    return;
  }
  ...
  if (!Number.isInteger(limit) || limit < 1) {
    throw new Error(`MAX_LLM_CALLS must be a positive integer, got "${raw}"`);
  }
  if (meteredTextCalls >= limit) throw new ExtractionBudgetError(limit);
  meteredTextCalls++;
}

export function isExtractionBudgetError(err) {
  return err?.code === 'EXTRACTION_BUDGET_EXHAUSTED';
}

export function isProviderQuotaExhausted(err) {
  const message = String(err?.message || err);
  return /prepayment credits are depleted|specified API usage limits|billing quota.*exhausted|quota.*exhausted.*billing/i.test(message);
}
...
export function scanPrompt(context) {
...
- Gib IMMER jedes Feld zurück, das du sicher entziffern kannst — auch bei schlechter Bildqualität, verwackelt oder teils verdeckt. Ein teilweise gelesenes Event, das der Nutzer fertig ausfüllt, ist besser als ein leeres Formular. Nicht lesbare oder nur vermutete Felder bleiben null; niemals raten.
...
// For the local crawl box (docs/ops/local-box-setup.md): a local model handles
// the LLM-fallback leftovers at $0 with no rate limits and no data leaving the
// machine. Falls through to Gemini→Claude on any failure, so a stopped Ollama
// never breaks a crawl. OLLAMA_URL default matches Ollama's standard port.
...
// Pin the context; never let Ollama auto-size it. It sizes the window to the
// model's trained max and the KV cache to match, and the cache is what decides
// whether the model still fits in VRAM. Measured on the box (RTX 4070 Ti SUPER,
// 16 GB): qwen2.5:14b auto-picked 32768 → 18 GB → 12 of 49 layers spill to CPU
// → 11.3 tok/s; pinned to 16384 it fits → 60.6 tok/s for identical output.
// gemma4's attention makes its cache cheap enough that 32768 costs nothing
// (8.4 GB / 100% GPU at both 16k and 32k), and the headroom is load-bearing:
// output shares this window, so a dense listing that needs ~10k tokens of JSON
// truncates mid-string at 16384 (done_reason=length → unparseable).
...
async function callOllamaText(pageText, system) {
  const res = await fetch(`${OLLAMA_BASE}/api/chat`, {
    ...
    body: JSON.stringify({
      ...
      // A JSON SCHEMA, not format:'json' — Ollama compiles it to a GBNF grammar
      // and constrains every token, which is why this provider (unlike Gemini,
      // whose OpenAPI subset rejects our ["string","null"] dialect) can enforce
      // the contract instead of asking for it. Measured: format:'json' let
      // qwen2.5 emit categories like "Diverse Musikveranstaltungen" — outside
      // the enum, i.e. junk crawl.mjs would carry into the DB. Grammar-level
      // enforcement makes that unrepresentable. It does NOT prevent fabrication.
      format: CRAWL_SCHEMA,
      stream: false,
      // Current models think by DEFAULT — gemma4 emitted 7.9k chars of reasoning
      // here, qwen3.5 22.7k — and that re-feeds as input: prompt tokens ~5×, the
      // context fills, and the JSON truncates. Off for everyone (verified
      // accepted by every model tested, thinking-capable or not). Don't remove.
      think: false,
      options: { temperature: 0, num_ctx: OLLAMA_NUM_CTX },
    }),
    // Local inference is slow and this only ever runs in the nightly batch, so
    // the ceiling is generous: a dense listing (gotoburgas.com — ~112 events on
    // one page) takes ~200s on the box and blew the old 180s, which silently
    // handed the page back to paid Gemini — the one outcome this provider exists
    // to avoid. A hung Ollama still can't wedge the crawl: it falls through.
    signal: AbortSignal.timeout(Number(process.env.OLLAMA_TIMEOUT_MS || 600000)),
  });
  if (!res.ok) throw new Error(`Ollama HTTP ${res.status}: ${(await res.text()).slice(0, 200)}`);
  const data = await res.json();
  // Both ends of this window fail SILENTLY, and neither looks like an error:
  // input past num_ctx is truncated (events in the cut tail simply never exist —
  // a halved page and a short page are indistinguishable downstream), and output
  // that hits the ceiling stops mid-token with done_reason='length', which the
  // grammar cannot save because valid JSON cut in half is still invalid. Name
  // which one happened rather than letting parseJson throw a position number
  // ("record WHAT failed", tasks/lessons.md). German runs ~3.2 chars/token,
  // Bulgarian ~2.3, so a 60k-char page can want ~26k tokens on its own.
  if (data.prompt_eval_count >= OLLAMA_NUM_CTX - 8) {
    console.error(`Ollama: page filled the ${OLLAMA_NUM_CTX}-token context (${data.prompt_eval_count} tokens) — tail truncated, events may be missing. Raise OLLAMA_NUM_CTX.`);
  } else if (data.done_reason === 'length') {
    console.error(`Ollama: ran out of context mid-answer (${data.prompt_eval_count} in + ${data.eval_count} out ≥ ${OLLAMA_NUM_CTX}) — JSON is truncated. Raise OLLAMA_NUM_CTX.`);
  }
...
function isTransient(err) {
  if (isExtractionBudgetError(err) || isProviderQuotaExhausted(err)) return false;
...
export async function extractFromPage({ text, sourceName, town, country = 'AT' }) {
  const context = {
    AT: { countryName: 'Österreich', timeZone: 'Europe/Vienna' },
    BG: { countryName: 'България', timeZone: 'Europe/Sofia' },
    DE: { countryName: 'Deutschland', timeZone: 'Europe/Berlin' },
  }[country] || { countryName: country, timeZone: 'UTC' };
  const today = new Intl.DateTimeFormat('en-CA', { timeZone: context.timeZone }).format(new Date());
  const system = `Du extrahierst Veranstaltungen aus dem Text einer Website (${sourceName}${town ? `, Ort ${town}` : ''}) für eine lokale Event-Karte in ${context.countryName}.
Heute ist ${today} (${context.timeZone}). Extrahiere NUR zukünftige Events mit erkennbarem Datum (bis ~8 Wochen voraus). Keine Fakten erfinden; unbekannte Felder = null. description: 1 kurzer Satz in eigenen Worten, in der Sprache der Quelle — nie den Text der Website kopieren. Navigations-Reste, vergangene Events und Nicht-Events ignorieren. Maximal 25 Events.
${PARTY_HINT}
Sprache/Schrift: Text-Felder (title, venue, address, town, description) IMMER in der Sprache und Schrift der Quelle wiedergeben — niemals übersetzen oder transliterieren (z.B. bulgarischer Text bleibt in kyrillischer Schrift, nie in lateinische Buchstaben umschreiben). Datumsangaben in jedem lokalen Format erkennen (z.B. "Tag.Monat.Jahr" mit landessprachigen Monatsnamen, kyrillischen Monatsnamen für bulgarische Quellen) und in date_start/date_end (YYYY-MM-DD) auflösen — Zeit als lokale Wanduhrzeit der Quelle, keine UTC-Umrechnung.`;
  ...
  if (geminiAvailable()) {
    try {
      // Batch crawl: a 429/5xx blip on one page must not burn the whole round
      // for that source — retry transient failures before falling back.
      return await withRetry(() => callGeminiText(text, system), 3);
    } catch (err) {
      if (process.env.EXTRACT_FALLBACK === 'none') throw err;
      console.error('Gemini text extraction failed, falling back to Claude:', err?.message);
    }
  }
  if (process.env.EXTRACT_FALLBACK === 'none') {
    throw new Error('Gemini text extraction unavailable and EXTRACT_FALLBACK=none');
  }
  return await withRetry(() => callClaudeText(text, system));
}
...
// Final postcondition for the probabilistic crawl lane. Dates and times must be
// real local wall-clock fields, and a description copied verbatim from source
// text is discarded rather than publishing prohibited prose.
export function sanitizeExtractedEvents(events, sourceText = '') {
  const source = normalizedProse(sourceText);
  const time = (value) => /^(?:[01]\d|2[0-3]):[0-5]\d$/.test(value || '') ? value : null;
  return (Array.isArray(events) ? events : []).flatMap((raw) => {
    const dateStart = validDateOf(raw?.date_start);
    if (!raw?.title || !dateStart) return [];
    const dateEnd = validDateOf(raw.date_end);
    const description = typeof raw.description === 'string' ? raw.description.trim() : null;
    const normalizedDescription = normalizedProse(description);
    const copied = normalizedDescription.length >= 30 && source.includes(normalizedDescription);
    return [{
      ...raw,
      date_start: dateStart,
      time_start: time(raw.time_start),
      date_end: dateEnd,
      time_end: dateEnd ? time(raw.time_end) : null,
      description: description && !copied ? description : null,
    }];
  });
}
...
function digestPrompt({ city, lang, weekendLabel }) {
  ...
  return `Du schreibst den wöchentlichen Wochenend-Newsletter von Okolo (okolo.events) — eine Event-Karte für die Region. Ausgabe für ${city}, Wochenende ${weekendLabel}.

PUBLIKUM: zwei Hälften, und der Newsletter ist in zwei Abschnitte geteilt — "Für Familien" (Eltern mit Kindern) und "Für alle" (u.a. junge Leute ohne Kinder, die Kunst, Musik und Kultur entdecken wollen). Schreib für BEIDE. Das subject und das intro dürfen sich NICHT nur an Familien richten — sonst widerspricht die Ansprache der halben Liste. Jedes Event trägt ein Feld "section" ("family" oder "all"), damit du weißt, wer gemeint ist; nenne die Abschnitte aber nicht wörtlich im intro.
