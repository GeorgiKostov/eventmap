...
export function decodeEntities(s) {
  ...
  let out = String(s);
  // Decode until stable. decodeOnce is monotonic (it only ever turns entities
  // into characters, never the reverse), so this can't oscillate; the small cap
  // is just a backstop against a pathological input, and handles the deeper
  // multi-encoding ("&amp;amp;#8211;") that a fixed 2 passes left half-decoded.
  for (let i = 0; i < 5; i++) {
