...
const T = {
  de: {
    ...
    unsub: 'Abmelden',
    preferences: 'Ausgabe oder Stadt ändern',
  ...
  bg: {
    ...
    unsub: 'Отписване',
    preferences: 'Промени изданието или града',
  ...
  en: {
    ...
    unsub: 'Unsubscribe',
    preferences: 'Change edition or city',
...
export function digestItem(e, { window, channel, section = null, teaser = null }) {
  return {
    id: String(e.id),
    title: e.title,
    ...
    // Automatic delivery rechecks this version immediately before sending. A
    // changed source row means the frozen wording may now be stale, so the job
    // fails closed and leaves the issue for operator review.
    eventUpdatedAt: e.updated_at
...
export async function buildDigest(channel, { limit = DIGEST_MAX, now = new Date(), ai = true, exclude = [] } = {}) {
  ...
  return {
    ...
    window,
    label,
    // The Thursday send job accepts only a snapshot freshly rebuilt that day;
    // SEO/page generation may freeze future weekends weeks in advance.
...
export function applyReplace(digest, id, pool) {
  ...
  items[idx] = digestItem(candidate, { window: digest.window, channel: digest.channel, section });
  return {
    ...digest,
    items,
    droppedIds: [...dropped],
...
export function renderNewsletter(digest, { unsubscribeUrl, preferencesUrl, mapUrl } = {}) {
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${PAPER};padding:24px 12px">
  <tr><td align="center">
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;padding:18px 26px">
      <tr><td style="font:400 12px/1.6 ${FONT};color:#8A938F;text-align:center">
        ${esc(c.why)}<br>
        ${preferencesUrl ? `<a href="${preferencesUrl}" style="color:#8A938F;text-decoration:underline">${esc(c.preferences)}</a> · ` : ''}<a href="${unsubscribeUrl}" style="color:#8A938F;text-decoration:underline">${esc(c.unsub)}</a> · <a href="${BASE}/impressum" style="color:#8A938F;text-decoration:underline">Impressum</a> · <a href="${BASE}/datenschutz" style="color:#8A938F;text-decoration:underline">${channel.lang === 'bg' ? 'Поверителност' : 'Datenschutz'}</a>
      </td></tr>
    </table>
  ...
  const text = [
    ...
    '',
    c.why,
    ...(preferencesUrl ? [`${c.preferences}: ${preferencesUrl}`] : []),
