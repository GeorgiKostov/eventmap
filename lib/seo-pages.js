...
export function canonicalSeoPath(pathname) {
  if (pathname === '/events/vienna' || pathname.startsWith('/events/vienna/')) {
    return pathname.replace('/events/vienna', '/events/wien');
  }
  return null;
}
...
export function seoDateRangeLabel(from, to = from) {
  const parse = (value) => {
  ...
  const month = new Intl.DateTimeFormat('de-AT', { month: 'long', timeZone: 'UTC' });
  ...
  if (start.getUTCFullYear() === end.getUTCFullYear()
    ...
    return `${start.getUTCDate()}.–${end.getUTCDate()}. ${monthYear.format(end)}`;
  }
  if (start.getUTCFullYear() === end.getUTCFullYear()) {
    return `${start.getUTCDate()}. ${month.format(start)}–${full.format(end)}`;
  }
...
export function availableDigestItems(digest, events) {
  if (!digest?.items?.length) return [];
  const liveIds = new Set(events.map((event) => String(event.id)));
  return digest.items.filter((item) => liveIds.has(String(item.id)));
}
