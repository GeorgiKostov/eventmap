...
import OkoloBrand from '../app/okolo-brand.js';
...
export function AdminShell({ title, subtitle, children }) {
  ...
  if (authed === null) {
    return (
      <div style={S.page}>
        <div style={{ ...S.wrap, ...S.card }}><OkoloBrand qualifier="admin" className="admin-brand" /><p style={S.muted}>…</p></div>
      </div>
    );
  }
    ...
    return (
      <div style={S.page}>
        <div style={{ maxWidth: 380, marginLeft: 'auto', marginRight: 'auto', ...S.card, marginTop: '12vh' }}>
          <OkoloBrand qualifier="admin" className="admin-brand" />
