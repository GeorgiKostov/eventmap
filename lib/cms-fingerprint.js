...
export const ROUTABLE_CMS = new Set([
  ...
  'wordpress-ical', 'naturfreunde', 'salzburgarena',
  'grazer-spielstaetten', 'bregenzer-festspiele',
  'posthof', 'rockhouse', 'brucknerhaus', 'kapu', 'tabakfabrik',
  'schlachthof-wels', 'twohop', 'last-space', 'florentine',
]);
...
export function fingerprintCms(html, url) {
  ...
  try {
    ...
    if (host === 'salzburgarena.at') return { cms: 'salzburgarena', signal: 'salzburgarena.at host' };
    if (host === 'spielstaetten.buehnen-graz.com') return { cms: 'grazer-spielstaetten', signal: 'Grazer Spielstätten host' };
    if (host === 'bregenzerfestspiele.com') return { cms: 'bregenzer-festspiele', signal: 'Bregenzer Festspiele host' };
    if (host === 'posthof.at') return { cms: 'posthof', signal: 'Posthof host' };
    if (host === 'rockhouse.at') return { cms: 'rockhouse', signal: 'Rockhouse host' };
    if (host === 'brucknerhaus.at') return { cms: 'brucknerhaus', signal: 'Brucknerhaus host' };
    if (host === 'kapu.or.at') return { cms: 'kapu', signal: 'KAPU host' };
    if (host === 'tabakfabrik-linz.at') return { cms: 'tabakfabrik', signal: 'Tabakfabrik host' };
    if (host === 'schlachthofwels.at') return { cms: 'schlachthof-wels', signal: 'Alter Schlachthof Wels host' };
    if (host === 'last-space.at') return { cms: 'last-space', signal: 'last-space.at host' };
    if (host === 'ccsmaragd.at') return { cms: 'twohop', signal: 'CulturCafé Smaragd host' };
    if (host === 'frl-florentine.at') return { cms: 'florentine', signal: 'Fräulein Florentine host' };
