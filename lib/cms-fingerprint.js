...
// crawl.mjs (tryStructuredExtraction tries them on every source regardless of
// `cms`), so finding one here on a source currently stuck on the LLM route
// means either the listing page changed since the last crawl, or the signal
// lives on a page other than the registered listing URL (a linked calendar
// view, for instance) — worth a note, not a `cms` change.
