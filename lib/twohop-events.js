...
export const TWOHOP_SOURCES = {
  ...
  // Official CulturCafé Smaragd calendar. The page also links date archives
  // under /events/YYYY-MM-DD/; only named detail pages carry Event JSON-LD.
  'https://www.ccsmaragd.at/veranstaltungen-linz': {
    town: 'Linz',
    pageParam: 'page',
    firstPage: 0,
    detailRe: /href=["'](https:\/\/www\.ccsmaragd\.at\/events\/(?!\d{4}-\d{2}-\d{2}\/)[^"'?#]+\/)["']/g,
    defaultVenue: 'CulturCafé Smaragd',
    defaultAddress: 'Altstadt 2',
  },
};
...
export async function fetchTwoHopEvents(src, {
  ...
  for (let i = 0; i < maxPages && details.size < maxDetails; i++) {
    ...
    let html;
    try {
      if (!await robotsFn(listUrl)) break;
  ...
  for (const url of details) {
    let html;
    try {
      if (!await robotsFn(url)) continue;
    ...
    for (const ev of parseJsonLdEvents(html, { town })) {
      ...
      const e = {
        ...ev,
        venue: ev.venue || cfg.defaultVenue || null,
        address: ev.address || cfg.defaultAddress || null,
        town: ev.town || town,
