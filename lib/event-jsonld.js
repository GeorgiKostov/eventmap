...
export function eventDescription(ev) {
  ...
  return `${ev.title}${place ? ` in ${place}` : ''}${date ? ` on ${date}` : ''}.`;
}
...
export function eventJsonLd(ev, id = ev.id) {
  ...
  return {
    ...
    startDate: ev.all_day || !hasTime(ev.starts_at)
      ? ev.starts_at.slice(0, 10)
      : isoLocalEventTime(ev.starts_at, timeZone),
    endDate: ev.ends_at
      ? (ev.all_day || !hasTime(ev.ends_at)
          ? ev.ends_at.slice(0, 10)
