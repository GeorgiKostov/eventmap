...
//
// ONE Meta Business account, but one IG/FB TARGET PER CITY: every entry point
// here takes a `channel` (a row from lib/city-channels.js) and posts to that
// channel's own fbPageId/igUserId. This is not decoration — the ids used to come
// from flat env vars, so a publish to ?channel=wien built a Vienna digest,
// rendered Vienna cards, posted the lot to the LINZ accounts and reported
// success. A channel is therefore REQUIRED, and a channel whose id is null is a
// hard failure, never a fall back to whatever env happened to hold.
//
// Env:
//   META_ACCESS_TOKEN   — Business Manager SYSTEM USER token (non-expiring),
//                         scopes: instagram_basic, instagram_content_publish,
//                         pages_manage_posts, pages_read_engagement.
//                         The token is shared by all channels; only the ids differ.
//   META_GRAPH_VERSION  — optional, default 'v23.0'.
//
// socialConfigured(channel) tells callers what's usable for THAT channel;
// missingSocialConfig(target, channel) names what to set for an honest 503.
// publishInstagramCarousel/publishFacebookPost either return a real
// { id, permalink } or throw — they never return a fake success. Ledger writes
// belong to the CALLER, and only after these resolve.
...
// What this CHANNEL can actually post to right now. A channel with no id for a
// surface is reported false — the desk must show "no account" rather than an
// enabled button that would post somewhere else.
export function socialConfigured(channel) {
  const token = !!process.env.META_ACCESS_TOKEN;
  return {
    instagram: token && !!channel?.igUserId,
    facebook: token && !!channel?.fbPageId,
  };
}

// Exactly what is missing, for honest error messages (mirrors the mail 503
// style — George needs to know precisely what to set). Names the env var when
// it's the shared token, and the registry field when it's the per-city id,
// because those are fixed in two different places.
export function missingSocialConfig(target, channel) {
  const missing = [];
  if (!process.env.META_ACCESS_TOKEN) missing.push('META_ACCESS_TOKEN');
  const field = target === 'instagram' ? 'igUserId' : 'fbPageId';
  if (!channel?.[field]) {
    missing.push(`${field} for '${channel?.slug ?? 'unknown'}' in lib/city-channels.js`);
  }
  return missing;
}
...
export async function publishInstagramCarousel({ channel, imageUrls, caption }) {
  const igUserId = channel?.igUserId;
  if (!igUserId) {
    throw new Error(
      `publishInstagramCarousel: channel '${channel?.slug ?? '(none)'}' has no igUserId — set it in lib/city-channels.js. Refusing to post: without it there is no target but another city's account.`,
    );
  }
...
export async function publishFacebookPost({ channel, imageUrls, message, link }) {
  const pageId = channel?.fbPageId;
  if (!pageId) {
    throw new Error(
      `publishFacebookPost: channel '${channel?.slug ?? '(none)'}' has no fbPageId — set it in lib/city-channels.js. Refusing to post: without it there is no target but another city's Page.`,
    );
  }
...
async function publishWithLedger({ channel, ledgerKey, imageUrls, caption, target, force, metaGet, metaSet, metaClaim, metaDelete, snapshotCheck }) {
  if (!force) {
  ...
  try {
    let result;
    if (target === 'instagram') {
      result = await publishInstagramCarousel({ channel, imageUrls, caption });
    } else if (target === 'facebook') {
      result = await publishFacebookPost({ channel, imageUrls, message: caption });
    } else {
...
export async function publishAndLedger({ channel, digest, target, force, metaGet, metaSet, metaClaim, metaDelete }) {
  ...
  return publishWithLedger({
    channel, ledgerKey, imageUrls, caption, target, force, metaGet, metaSet, metaClaim, metaDelete,
...
export async function publishItemAndLedger({ channel, digest, item, target, force, metaGet, metaSet, metaClaim, metaDelete }) {
  ...
  return publishWithLedger({ channel, ledgerKey, imageUrls, caption, target, force, metaGet, metaSet, metaClaim, metaDelete });
}

// One-off/editorial carousels use the same target checks, atomic claim, and
// success-only ledger as the weekly digest without pretending to be part of a
// frozen weekend snapshot. The caller owns the verified public image URLs and
// caption; `slug + date` gives each editorial package an independent,
// retry-safe key on Instagram and Facebook.
export async function publishEditorialCarouselAndLedger({
  channel, slug, date, imageUrls, caption, target, force,
  metaGet, metaSet, metaClaim, metaDelete,
}) {
  if (!/^[a-z0-9-]+$/.test(slug || '')) {
    throw new Error('publishEditorialCarouselAndLedger: slug must contain only lowercase letters, numbers, and hyphens');
  }
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date || '')) {
    throw new Error('publishEditorialCarouselAndLedger: date must be YYYY-MM-DD');
  }
  const kind = target === 'instagram' ? 'ig' : target === 'facebook' ? 'fb' : null;
  if (!kind) throw new Error(`publishEditorialCarouselAndLedger: unknown target ${target}`);
  const ledgerKey = `posted:${kind}:${channel.slug}:${date}:editorial:${slug}`;
  return publishWithLedger({
    channel, ledgerKey, imageUrls, caption, target, force,
    metaGet, metaSet, metaClaim, metaDelete,
  });
}
