...
// access here (callers supply a bounded same-day/location candidate set from
...
// Stricter bar for AUTOMATIC merge paths (the crawl): templated municipal
// titles ("Josefstadt spielt 2026 – Kostenlose Veranstaltungen im Sommer" vs
// "Meidling spielt 2026 – …") clear titlesMatch's Jaccard 0.75 while naming
// different real events. A genuine cross-source duplicate EXTENDS the same
// title (one side adds tokens: "Imagine…" → "Imagine… – Kultursommer Wien");
// two different events SUBSTITUTE one content word for another (Josefstadt ↔
// Meidling). Returns true when BOTH titles carry tokens the other lacks — the
// substitution shape. Review-mediated paths (scan UI, merge-dups dry run) keep
// the looser titlesMatch on purpose; a human sees the pair before anything is
// destroyed.
export function titleSubstitution(titleA, titleB) {
  const a = tokenSet(titleA);
  const b = tokenSet(titleB);
  let onlyA = false;
  let onlyB = false;
  for (const t of a) if (!b.has(t)) { onlyA = true; break; }
  for (const t of b) if (!a.has(t)) { onlyB = true; break; }
  return onlyA && onlyB;
}
