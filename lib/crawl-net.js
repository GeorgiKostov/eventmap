...
export const UA = 'OkoloBot/1.0 (+https://okolo.events; event facts indexing with linkback; contact: hello@okolo.events)';
...
export async function politeFetch(url, opts = {}) {
  const u = new URL(url);
  const { robotsRequest = false, redirectCount = 0, ...fetchOpts } = opts;
  ...
  if (res.status >= 300 && res.status < 400 && res.headers.get('location')) {
    if (redirectCount >= 5) throw new Error(`Too many redirects fetching ${url}`);
    const next = new URL(res.headers.get('location'), u).toString();
    // Permission for the original URL does not authorize a redirected host or
    // path. Robots-file redirects are the exception: their response IS the
    // policy being retrieved and cannot recursively require itself first.
    if (!robotsRequest && !await robotsAllowed(next)) {
      throw new Error(`Redirect target disallowed by robots.txt: ${next}`);
    }
    return politeFetch(next, { ...fetchOpts, robotsRequest, redirectCount: redirectCount + 1 });
  }
  return res;
}
...
async function robotsGroups(origin) {
  if (!robotsCache.has(origin)) {
    let groups = null;
    try {
      const res = await politeFetch(`${origin}/robots.txt`, { robotsRequest: true });
      if (res.ok) groups = parseRobots(await res.text());
      // RFC 9309 §2.3.1.1: 4xx means the robots file is unavailable and the
      // crawler MAY access resources. 5xx is an unreachable robots service and
      // must fail closed for this run (§2.3.1.2), as must a network failure.
      else if (res.status >= 400 && res.status < 500) groups = [];
    } catch { /* unreachable robots service → fail closed */ }
...
// Historical named-AI classifier retained for audits and old decision evidence.
// It no longer gates Okolo: from 2026-08-20, permission is determined by the
// rules that actually apply to OkoloBot (or `*`). A ban on another crawler,
// such as CCBot or GPTBot, does not ban our factual indexer.
//
...
export async function robotsAllowed(url) {
  ...
  // RFC 9309 matching includes the query component. Ignoring it made rules such
  // as `Disallow: /*?print=1` ineffective on exactly the generated second hops.
  return !isDisallowed(group, `${u.pathname}${u.search}`);
}
